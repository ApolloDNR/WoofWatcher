...
- Report History owner-approval guidance now stays shared across Records and WoofGuide before native PDF/export, cloud sharing, or server-backed report storage. `summarizeReportArtifacts.approvalLine` tells owners to choose the handoff audience and approve the local source content before sharing, and the same line appears in Mobile Records plus the owner-reviewed WoofGuide report-history draft without claiming provider-backed lifecycle controls or unsupervised assistant actions are ready.
