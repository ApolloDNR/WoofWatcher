...
The 2026-06-20 mobile-layout pass centralized floating-paw tab chrome and route bottom spacing in `mobileLayout.ts`. The tab shell now uses shared tab/FAB metrics, and Home, Log, Plans, Health, More, and Records all use `getTabbedRouteBottomPadding` instead of route-local fixed padding. This reduces mobile safe-area drift before the native device pass, but real iOS/Android QA still must confirm phone-size bottom-nav clearance, touch reach, and route scrolling.
