...
As of 2026-06-11, the current queue points to mobile runtime QA and premium polish: native simulator/device smoke, full accessibility pass, visual regression, App Store prep, premium design/motion, provider-backed notification delivery, report export hardening, and richer medication history. CI-safe static route/safety smoke, critical action screen-reader label smoke, Expo web export smoke, Free/Plus/Family entitlement policy, release-grade Expo app identity, a tested Home avatar motion state model, print-ready Care Pass artifact HTML, Records report-history print-source sharing, printable Dog ID credential source, medication adherence status, medication-specific log defaults, Medication Follow-ups, and Care Pass medication report language exist, but native rendering, screen-reader traversal, screenshots, final Rive/Lottie/Reanimated assets, binary PDF generation, real push notifications, server-backed report storage, EAS/App Store credentials, and real checkout remain blocked.
