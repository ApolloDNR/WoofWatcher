...
As of 2026-06-09, the current queue points to mobile runtime QA and premium polish: native simulator/device smoke, full accessibility pass, visual regression, App Store prep, premium design/motion, and report export hardening. CI-safe static route/safety smoke, critical action screen-reader label smoke, Expo web export smoke, Free/Plus/Family entitlement policy, release-grade Expo app identity, a tested Home avatar motion state model, print-ready Care Pass artifact HTML, Records report-history print-source sharing, and printable Dog ID credential source exist, but native rendering, screen-reader traversal, screenshots, final Rive/Lottie/Reanimated assets, binary PDF generation, server-backed report storage, EAS/App Store credentials, and real checkout remain blocked.
