...
### 2026-07-02: Report History Audience Prep Is Shared

Decision: Records Report History and WoofGuide saved-report drafts should reuse a shared audience-prep line from `summarizeReportArtifacts`. The line may tell owners to pick the sitter, trainer, caregiver, or vet audience first because each handoff may need different notes, but it must not imply provider-backed storage, native export, cloud sharing, retention, deletion, or unsupervised assistant actions are ready.

Reason: Report History now supports local reusable sources and pre-share review. Owners also need a consistent cue to choose the actual recipient before resending, so a vet-facing source does not accidentally go out with sitter-style notes or vice versa.
