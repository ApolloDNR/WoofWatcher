...
1. Auth-connected first-run onboarding for account provisioning, household invite/join flow, multi-household confirmation, and post-setup confirmation.
...
- Add broader role-aware authorization checks beyond basic household membership. Household rename and audit review now have owner/admin guards, and default household creation, rename, active-household switching, and invite acceptance write audit events, but invite approval, caregiver editing, document access, role changes, and other sensitive account/household actions still need provider-backed policy.
