...
- Add deeper multi-device conflict policy, live DB validation and final restore/retention rules for server-backed delete retention, and broader background retry recovery policy.
...
- Missing live API integration tests against a test database/provider-auth harness. Focused API route contract readiness now covers auth/scope/concurrency/query wiring without requiring secrets.
