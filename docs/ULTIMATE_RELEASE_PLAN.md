...
3. Provider-backed household invite approval lifecycle, audit review/admin APIs, scheduled Access Pass expiry cleanup, provider migration/RLS/retention rules for durable household audit rows, helper audit trails, and caregiver editing.
...
- Expand role-aware authorization beyond the new care-entry write policy, household role mutation/revocation API contracts, durable household audit row schema, invitation audit metadata, and Access Pass helper activation/revocation contracts into invite approval state storage, audit review/admin APIs, scheduled Access Pass expiry cleanup, provider migration/RLS/retention rules, and broader account-action audit events.
...
- Access Pass may stage local permission drafts for helpers, and the API now supports owner/admin activation and revocation of existing household helper roles with provider-durable audit row readiness and future-expiry validation. Invite approval lifecycle states, audit review/admin APIs, scheduled expiry cleanup, provider migration/RLS/retention rules, and production provider sync remain provider-gated until account/database rules are approved.
