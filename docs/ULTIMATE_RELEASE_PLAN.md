...
- API readiness now guards OpenAPI/generated-client coverage for WoofGuide events, Avatar Studio image routes, household provisioning/auth errors, `/care-entries?limit=...`, care-entry create/update/delete validation/not-found response shapes, and `PUT /care-state` validation/not-found/conflict response shapes; continue with behavior/integration tests for rate limits, provider-gated WoofGuide/action flows, and authenticated household scoping across write/read paths.
