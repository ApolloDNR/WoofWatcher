...
- Add live API integration tests for care entries, care state, household provisioning, WoofGuide events, and rate limits. Focused API route contract readiness now covers core auth/scope/concurrency/query wiring, including active-household scoping for member display-name updates and owner/admin household rename gating, without requiring secrets.
