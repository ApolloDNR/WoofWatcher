...
3. Provider-backed household invite approval lifecycle, scheduled Access Pass expiry cleanup, provider migration/RLS/retention rules for durable household audit rows, helper audit trails beyond existing audit review APIs, and caregiver editing.
...
- Expand role-aware authorization beyond the new care-entry write policy, household role mutation/revocation API contracts, durable household audit row schema, invitation audit metadata, Access Pass helper activation/revocation contracts, and owner/admin audit review APIs into invite approval state storage, scheduled Access Pass expiry cleanup, provider migration/RLS/retention rules, and broader account-action audit events.
...
- Access Pass may stage local permission drafts for helpers, and the API now supports owner/admin activation and revocation of existing household helper roles with provider-durable audit row readiness, future-expiry validation, and owner/admin audit review/list APIs. Invite approval lifecycle states, scheduled expiry cleanup, provider migration/RLS/retention rules, and production provider sync remain provider-gated until account/database rules are approved.
