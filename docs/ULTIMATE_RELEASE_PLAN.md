...
- The current source/test slice extends restricted-role recovery into one principal-safe lifecycle: unresolved auth is inert; token transport, query state, caches, and deletion intent are principal-scoped; verified `/me` household identity gates remote work. Each care request sends `x-woofwatcher-household-id`, and the server rejects disagreement with `409` before care-table access; invalidation retries through one outstanding two-second timer rather than a hot loop.
- Mismatched household state is masked and restored only for its exact archive. If archival fails and the same household returns, the full pending document, entries/outbox, synchronized baseline, and deletion ledger are durably restored before unmasking.
- Busy refreshes now coalesce, and document `409` recovery uses a three-way server/baseline/local merge with explicit conflict recovery. Serialized persistence separates critical cleanup from discardable snapshots; the truthful local-data clear completes avatar writes before its terminal care wipe and reports partial failure; hydration is erase-generation fenced. Entry deletion is serialized/generation-fenced, excludes edits across temp/server aliases, repairs failed tombstone clears atomically, persists a real-id tombstone before hiding, and suppresses stale-list ghosts. Every CareContext provider request is timeout-bounded, nullable-add callers cannot present blocked saves as success, and generated React/Zod care contracts are held current by CI.
...
- Release evidence still requires configured-provider owner/adult/restricted roles and RLS, live account/household switches, revocation and concurrent-device runs, offline recovery, real-iPhone behavior, VoiceOver/TalkBack and screenshots, store review, and Apollo sign-off.
...
- The current automation branch tip has fresh dependency-complete CI proof: `WoofWatcher Verify` run `28836909561`, job `85522525710`, passed on commit `d21f44e` with install, JSON mobile beta doctor, focused behavior tests, and Typecheck plus CI-safe builds. Share Beta Handoff records that proof while keeping native iOS/Android screenshots, provider proof files, live-preview rerun, store review, public launch, and Apollo sign-off blocked.
- Privacy & Safety now consumes saved Provider Launch Setup AI, payments, and account-deletion proof evidence instead of only configured booleans. The saved structured evidence can reach the existing WoofGuide AI, payments, and deletion validators, while real provider proof files, checkout, destructive deletion, public launch, and Apollo sign-off remain blocked.
- The focused Store Accounts proof mission now consumes saved Provider Launch Setup Apple/Google proof evidence instead of rendering an empty manifest. Real App Review or Play review readiness still requires platform/store-named Apple Developer/App Store Connect, Google Play, bundle/signing, reviewer access, metadata/privacy-label, Apollo release approval, and no-submit-boundary proof files.
- The focused Account Deletion proof mission now consumes saved Provider Launch Setup deletion/legal proof evidence instead of rendering an empty manifest. Real destructive deletion readiness still requires structured deletion-route/auth, export-before-delete, data/object deletion receipt, audit/support receipt, recovery/cancellation, legal/store approval, and Apollo approval proof files.
- The focused Support Legal Readiness proof mission now consumes saved Launch Support Profile support/legal proof evidence instead of rendering an empty manifest. Real public-launch readiness still requires structured support inbox, privacy policy and terms, refund/subscription, veterinary/emergency boundary, deletion escalation, incident response owner, Apollo launch approval/no-launch-boundary proof files, and Apollo sign-off.
...
- Premium and the focused Payments Provider Proof mission also consume saved Provider Launch Setup payments proof evidence, so real saved product catalog, billing path, receipt/restore, entitlement, refund/support, and checkout-gate proof can be reviewed without enabling checkout or money movement.
...
- Focused Care-entry Provider Sync and WoofGuide AI proof missions now consume saved Provider Launch Setup proof evidence instead of rendering empty manifests. Valid saved Supabase/RLS/migration proof can reach `deriveCareEntryProviderSyncProof`, and valid saved AI proof can reach `buildAiProviderProofManifest`, but incremental sync and live AI remain blocked until the structured proof manifests are complete and Apollo signs off.
