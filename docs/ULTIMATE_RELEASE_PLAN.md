...
3. Provider-backed household invite approval lifecycle, durable Access Pass audit storage, Access Pass expiry enforcement, helper audit trails, and caregiver editing.
...
- Expand role-aware authorization beyond the new care-entry write policy, household role mutation/revocation API contracts, invitation audit metadata, and Access Pass helper activation/revocation contracts into durable provider-backed invite lifecycle states, Access Pass expiry enforcement, helper audit storage, and account-action audit events.
...
- Access Pass may stage local permission drafts for helpers, and the API now supports owner/admin activation and revocation of existing household helper roles with response-level audit metadata. Durable audit storage, expiry enforcement, invite approval lifecycle states, and production provider sync remain provider-gated until account/database rules are approved.
