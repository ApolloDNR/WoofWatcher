...
| Offline/Sync | Mobile care sync handles local/pending/failed status, retry separation, a durable outbox summary, CareContext exposure, a Log screen recovery banner with retry counts/action, and a More Sync Health dashboard for household sync status, care-log count, team count, outbox count, and retry/refresh guidance. | Add conflict-safe state updates, native runtime recovery QA, and broader recovery tests. |
...
| CI/QA | GitHub Actions verifies install, focused tests, typecheck, API/web builds, mobile Expo web export smoke, and can be manually dispatched if a push hook misses. Focused tests include static mobile readiness smoke for route registration, tabs, router links, launch-blocking safety copy, CI smoke wiring, critical action accessibility labels, release-grade app identity, Home avatar motion wiring, hydration visibility from Home quick log to Records, Walk Activity wiring, Potty Health wiring, durable sync outbox wiring, and household Sync Health wiring. | Add native simulator/device smoke, API route tests, Playwright or simulator screenshot checks, full accessibility traversal, and visual regression where feasible. |
...
5. Add conflict-safe care state recovery and native offline recovery QA.
