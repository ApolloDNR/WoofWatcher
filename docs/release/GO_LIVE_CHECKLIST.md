...
- [x] A1. Expo app configured for release: `artifacts/woofwatcher-mobile/app.json` with app name WoofWatcher, version 1.0.0, iOS bundle id and Android package `com.pegasusdreamscapes.woofwatcher`, icon/splash assets, and camera/photo/foreground-location permission strings (the only permissions declared; location is used solely to draw the on-device walk map).
