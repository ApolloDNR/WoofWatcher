...
- Share Beta Handoff now includes recorded branch `WoofWatcher Verify` proof for run `28661364982`, job `85002511278`, commit `38ae755`, while explicitly requiring a rerun after any new commit and saying CI proof does not approve native screenshots, provider setup, store approval, or Apollo sign-off.
...
- Need exact Plus/Family product ids, billing path decision, sandbox purchase/renewal/cancel/refund/expired receipt proof, restore-purchase proof, entitlement mapping, refund/support policy, store checkout approval, and Apollo sign-off before paid checkout can be enabled.
- Need real Apple Developer/App Store Connect access, Google Play package record, reviewer credentials, screenshots/metadata/privacy-label approval, release-role approval, and Apollo store-submission sign-off before App Review or Play review can be claimed.
