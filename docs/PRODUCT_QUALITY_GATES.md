...
- The Route Visual proof manifest now requires route-named iOS/Android evidence per route instead of treating generic platform screenshot counts as complete proof. Generic six-iOS/six-Android attachments still count in the summary, but Home, Log, Plans, Health, Records, and More stay blocked until the evidence file name or URI names that route for that platform. This keeps visual proof blocked until actual route screenshots and human review exist.
