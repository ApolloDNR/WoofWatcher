...
- Records and WoofGuide report handoff prep now reuse `summarizeReportHandoffPrep`, so owners see one checklist for saved local report sources, local receipt/document attachment readiness, and Dog ID credential readiness before resending or sharing printable source, without claiming provider-backed storage, native export, cloud sharing, retention, deletion, or unsupervised assistant actions are enabled.
