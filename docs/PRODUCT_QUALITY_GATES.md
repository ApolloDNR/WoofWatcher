...
- Care Pass now includes Dog ID Prep lines from the same shared credential readiness, so sitter/vet/trainer/caregiver handoffs show which credential fields are ready or missing without claiming image/PDF export or provider-backed credential storage is ready.
