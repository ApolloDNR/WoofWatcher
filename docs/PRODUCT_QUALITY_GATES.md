...
- `/care-twin-qa?qaSurface=report-binary-export-proof` now turns that packet into a focused launch-critical QA target for approved generator choices, generated file name/size/MIME/share proof, provider storage policy, and iOS/Android artifact evidence.
...
- `build:ci` now runs the mobile `smoke:web` export, `smoke:runtime`, and `proof:live-preview`; the runtime smoke verifies 11 exported routes return the Expo web shell, and the live-preview proof verifies 9 launch-critical preview handoff routes with web-preview-only truth boundaries.
...
- Share Beta Handoff now includes recorded branch `WoofWatcher Verify` proof for run `28657836139`, job `84991128098`, commit `5f80049`, while explicitly requiring a rerun after any new commit and saying CI proof does not approve native screenshots, provider setup, store approval, or Apollo sign-off.
