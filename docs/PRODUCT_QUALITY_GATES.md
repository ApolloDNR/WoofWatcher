...
- The Support runbook now consumes the Support legal readiness proof manifest for its public-launch verdict. Support/legal approval booleans can stage owner review, but `launchReady`, `supportRunbookApproved`, and `privacyLegalApproved` remain blocked until structured support/legal proof files make the manifest ready. Branch CI proved this Support runbook guard in `WoofWatcher Verify` run `28705194968`, job `85129614020`, on commit `ceecc55`.
