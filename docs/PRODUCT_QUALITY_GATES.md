...
- Privacy & Safety now consumes the Payments provider proof manifest for its payments gate. `paymentsEnabled` can stage provider setup, but checkout remains `blocked` until structured payments proof files make the manifest ready.
