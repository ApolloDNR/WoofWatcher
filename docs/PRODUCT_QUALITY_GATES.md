...
- API shared care writes now enforce launch role boundaries before final provider-backed permission policy. Owner, admin, and member roles can update the shared care plan through `PUT /care-state`; sitter and trainer roles can still log and correct care evidence through `POST/PATCH/DELETE /care-entries`; vet viewers receive a clear 403 on both care-plan and care-log writes.
