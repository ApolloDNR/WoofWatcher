...
- API shared care writes now enforce the vet viewer read-only boundary before final provider-backed permission policy. Owner, admin, member, sitter, and trainer roles can mutate care plans and logs, while vet viewers receive a clear 403 on `PUT /care-state` and `POST/PATCH/DELETE /care-entries`.
