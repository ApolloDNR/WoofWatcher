...
- WoofGuide shows deterministic suggested actions tied to health watch, record attention, diet setup, routines, meal logging, Care Pass preview, owner-reviewed Mood & Energy summaries from shared mood evidence, owner-reviewed Records Attachment Prep from local receipt/document readiness, and owner-reviewed Dog ID Prep from shared credential readiness.
...
- Records Dog ID shares now save local `pet_credential` artifacts in Report History. `createPetCredentialArtifact` stores escaped printable Dog ID HTML and stable file names, while Mobile Records labels saved rows as Dog ID Credential without claiming image/PDF export, provider-backed credential storage, cloud sharing, or server-backed report storage is ready.
- Records Report History now shows local handoff source readiness across Care Pass, Progress Report, and Dog ID artifacts. `summarizeReportArtifacts` counts the saved source mix, identifies the latest reusable source, and Mobile Records repeats that native PDF export, server-backed report storage, cloud sharing, retention, and deletion policy are not enabled.
- WoofGuide saved report-history readiness reuses the same `summarizeReportArtifacts` boundary. When local Care Pass or Progress Report sources exist, the owner-reviewed `report_history` draft routes owners to Records Report History for resend or printable-source sharing and inserts only reviewed assistant text without claiming native PDF export, server-backed report storage, cloud sharing, retention, deletion, or unsupervised assistant actions are ready.
- Records Report History rows and WoofGuide saved report-history drafts now reuse `describeReportArtifactSource` for the latest source label, section count, print-ready/restored status, printable file name, and local-only lifecycle copy, keeping native PDF export, server-backed report storage, cloud sharing, retention, and deletion policy gated in both surfaces.
- Records and WoofGuide saved-report review now reuse `summarizeReportArtifacts.readinessLine`, so owners are told to confirm the saved local source still has the audience's needed Dog ID fields, receipt/document attachments, and report sections as applicable before resending a Care Pass, Progress Report, or Dog ID source.
- Records and WoofGuide saved-report review now reuse `summarizeReportArtifacts.reviewLine`, so owners are told to review the latest local source for stale routines, medications, records, and audience before resending a Care Pass, Progress Report, or Dog ID source.
- Records and WoofGuide saved-report review now reuse `summarizeReportArtifacts.cleanupLine`, so cleanup guidance tells owners to remove obsolete local sources only after review and states that local cleanup does not revoke shares or change provider retention.
- Records Report History now uses `describeReportArtifactRemoval` to confirm removal of a selected local reusable source. The mobile action filters only that local artifact from the care document and states that cloud deletion, share revocation, server retention, native PDF export, and provider-backed lifecycle controls are not enabled.
...
- Care Pass and Progress Reports now include Dog ID Prep lines from the same shared credential readiness, so sitter/vet/trainer/caregiver handoffs and saved report-history artifacts show which credential fields are ready or missing without claiming image/PDF export, provider-backed credential storage, or server-backed report storage is ready.
...
- WoofGuide Dog ID Prep reuses the same shared credential readiness and stays non-mutating: applying the draft inserts only a reviewed assistant note, routes owners to Records, and states that image/PDF export plus provider-backed credential storage are not enabled.
- WoofGuide saved Dog ID history reuses local `pet_credential` report-history artifacts and stays non-mutating: applying the draft inserts only a reviewed assistant note, routes owners to Records Report History for resend/print-source sharing, and states that credential storage, native PDF/image export, cloud sharing, retention, and deletion policy are not enabled.
...
- Deterministic WoofGuide action-card model and tests exist before action writes are enabled, including owner-reviewed Mood & Energy summaries, Dog ID Prep drafts, and saved Dog ID history drafts that reuse shared evidence/readiness/report-history helpers without mutating care records.
...
- Progress Reports and direct Dog ID shares now persist print-ready report-history artifacts with the same Mood & Energy, Dog ID Prep, and local credential-source boundaries, so the paid report workflow is reusable before binary PDF generation, native credential export, provider-backed credential storage, or server-backed report storage exists.
...
- Need broader report/export tests beyond the current Care Pass artifact, print source, local Report History removal, and Mood & Energy handoff coverage.
