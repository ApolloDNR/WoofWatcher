...
- Progress Reports now persist print-ready report-history artifacts with the same Mood & Energy and Dog ID Prep boundaries, so the paid report workflow is reusable before binary PDF generation, native credential export, or server-backed report storage exists.
