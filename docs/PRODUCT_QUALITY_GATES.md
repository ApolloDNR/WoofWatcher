...
- Pack Audit role-change rows use owner-readable role labels and previous-to-new transition copy, avoiding internal role ids in visible rows and screen-reader labels before final provider-backed audit policy exists.
...
- Mobile Pack Audit role-change label readiness is covered so role updates render owner-readable previous-to-new details instead of leaking internal ids like `vet_viewer`.
