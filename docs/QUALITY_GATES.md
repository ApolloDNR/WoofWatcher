...
Current evidence, 2026-06-18: PixelLab asset verification now checks 81 assets across the full Phoenix sprite manifest, dogless room variants, 12 template previews, 12 template base stills, the 10-state Phoenix/Shepherd emote pack, the first non-Phoenix 10-state Retriever emote pack, the 10-state Husky/Spitz emote pack, the 10-item accessory inventory pack, and two subscription seed animation strips. Mobile readiness tests also verify Avatar Studio uses selected-template emote routing, applies crisp pixel image rendering, and uses the Studio presentation of `LivingPhoenixRoom` instead of the old static hero path.
