...
Current evidence, 2026-06-25: Mobile More now surfaces household audit review for owner/admin trust inspection. The Pack Audit board reads the generated household audit-events hook, lists recent pack creation, rename, active-household switching, and invite-join events, supports event-type and lifecycle filters through the existing `action` and `lifecycleState` query contract, and shows truthful loading/empty/offline states without exposing lifecycle actions before provider-backed retention policy exists.
