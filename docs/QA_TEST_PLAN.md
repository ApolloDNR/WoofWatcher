...
- PASS: API readiness now protects vet viewers as read-only on shared care writes. `PUT /care-state` and `POST/PATCH/DELETE /care-entries` use a shared `CARE_WRITE_ROLES` allowlist so owner/admin/member/sitter/trainer roles can mutate shared care while vet viewers receive a clear 403 and retain review-only access.
...
- Vet viewer read-only API enforcement for shared care writes, including `PUT /care-state` and `POST/PATCH/DELETE /care-entries` returning 403 through the shared care-write role guard while read/review routes remain available.
