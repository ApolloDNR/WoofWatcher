...
- LIMIT: Direct workspace TypeScript build still could not start on 2026-07-01 because `node_modules\typescript\bin\tsc` is missing in this detached checkout (`MODULE_NOT_FOUND`). Focused tests, PixelLab verification, and diff check passed locally; full CI remains the authoritative typecheck/build gate after push.
- PASS: GitHub Actions `WoofWatcher Verify` run `28514313498` on 2026-07-01 for commit `58d2381` completed the real `Install, Test, Typecheck, Build` job successfully after the Report History cleanup guidance slice.
...
- LIMIT: `pnpm run build:ci` on 2026-06-30 first failed in the sandbox with registry `EACCES`/`fetch failed`. The escalated retry hydrated all 1276 packages, then stopped at the root `preinstall` script because `sh` is not available on PATH before workspace typecheck/build scripts started.
...
- LIMIT: GitHub Actions `WoofWatcher Verify` run `28460926161` for commit `6233e99` failed in 4 seconds before executing the `Install, Test, Typecheck, Build` job. The job had `steps: []`, and `gh run view --log-failed` returned `log not found: 84348244511`, matching the standing pre-execution billing/spending-limit blocker rather than a code regression.
- LIMIT: `pnpm run build:ci` on 2026-06-30 could not complete in this Windows shell. The latest local attempt stopped during dependency hydration with registry `EACCES`, `fetch failed`, and `UND_ERR_DESTROYED` before workspace typecheck/build scripts started; earlier fuller attempts also stopped at the root `preinstall` script because `sh` is not available on PATH.
...
- PASS: Mobile readiness now protects the More active-household switcher. `/me` exposes the authenticated user's household list, generated contracts include `Me.households`, and the More Care Team surface uses `useSetActiveHousehold`, accessible selected/disabled state, pending/error copy, and care-state refresh after switching.
- PASS: API readiness now protects owner/admin household audit review. `household_audit_events` is exported from the DB schema, `GET /household/audit-events` is owner/admin gated, list queries normalize `limit`, `action`, and `lifecycleState`, and OpenAPI/zod/generated React client contracts expose the review endpoint.
- PASS: API readiness now protects sensitive household audit producers. Default household creation, household rename, active-household switching, and invite acceptance write durable `household_audit_events` rows with bounded action names and target details before final provider-backed account audit policy exists.
- PASS: API readiness now protects owner/admin household member role updates. `PATCH /household/members/{memberId}` is active-household scoped, rejects non-owner/admin callers, refuses owner demotion, writes durable `household.member_role_changed` audit rows, and stays documented across OpenAPI, zod, and the generated React client before full provider-backed caregiver administration exists.
- PASS: Mobile readiness now protects More Care Team role management. Existing synced non-owner members can be assigned admin, member, sitter, trainer, or vet viewer roles through the generated `useUpdateHouseholdMember` hook, role chips expose selected/disabled state, `/me` refreshes after success, Pack Audit refetches role-change events, and owner transfer/removal/invite approval remain provider-gated.
- PASS: Household Access now protects launch role labels and permission summaries for owner, admin, sitter, trainer, and vet viewer. Mobile readiness also protects More's visible Care Team permission line so internal role ids do not leak and final provider-backed enforcement remains a separate blocker.
- PASS: API readiness now protects vet viewers as read-only on shared care writes. Vet viewers receive a clear 403 on care-plan and care-log writes while retaining review-only access.
- PASS: API readiness now protects the split between care-plan and care-log write roles. `PUT /care-state` uses owner/admin/member only, while `POST/PATCH/DELETE /care-entries` still allows owner/admin/member/sitter/trainer roles to log and correct care evidence.
- PASS: API readiness now protects sitter/trainer self-correction boundaries. `PATCH /care-entries/:id` and `DELETE /care-entries/:id` include `caregiverUserId` scoping for sitter and trainer roles, while owner/admin/member roles keep household-wide correction authority.
- PASS: Mobile readiness now protects the More Pack Audit surface. The Care Team area uses the generated `useListHouseholdAuditEvents` hook, shows latest owner/admin household trust events, supports event-type and lifecycle filters against the existing audit-events query contract, summarizes stored creation/rename/switch/join/role-change details in owner-readable rows and screen-reader labels, and keeps loading, filtered-empty, empty, and offline/error states truthful while lifecycle changes and retention policy remain provider-gated.
- PASS: Mobile readiness now protects Pack Audit role-change details from leaking internal role ids. Role-change rows use owner-readable labels and previous-to-new copy such as Sitter to Vet viewer while older rows with only a new role still get a readable fallback.
- PASS: API and mobile readiness now protect target identity on role-change audit rows. `PATCH /household/members/{memberId}` stores the target member's display name/email in durable audit details, and More renders caregiver-specific copy such as `Emma: Sitter to Trainer` while preserving generic fallback copy for older rows.
...
- PASS: Mobile readiness now protects the setup-to-More handoff from lingering after the owner acts. More keeps `setupHandoff` as route context, renders through `visibleSetupHandoffIntent`, and calls `completeSetupHandoff(...)` when the owner shares the invite or opens the invite-code modal.
- PASS: Mobile readiness now protects invite-code join confirmation in More. Successful `useJoinHousehold` responses name the joined pack as the active care sync pack, refresh `/me` and care state, and keep provider-backed invite approval explicitly gated.
- PASS: Mobile readiness now protects active-pack-scoped display-name copy in More. The row, edit modal, and save confirmation name the selected household context so multi-household caregivers are not told the update applies to every household or provider-backed surface.
- PASS: Mobile readiness now protects the animated care-twin interaction. `AnimatedAvatar` exposes the full-scene tap as a screen-reader button, names Phoenix's current mood plus speech fallback, and makes clear the tap only plays a gentle response without changing care records.
...
- PASS: Care-domain and mobile readiness now protect report-ready Mood & Energy snapshots in Progress Reports. `deriveMoodEnergyReportSnapshot` reuses shared mood-trend evidence, excludes private/stale mood logs, includes energy counts plus latest caregiver/context, and Records includes the same owner-reported/non-diagnostic lines in the share payload.
- PASS: Care-domain and mobile readiness now protect saved Progress Report artifacts. `createProgressReportArtifact` stores escaped print-ready HTML, stable filenames, section metadata, and Mood & Energy boundary lines, while Records saves Progress Reports into Report History and uses one print-source path for Care Pass and Progress Report artifacts.
...
- PASS: Plans routine/event modal controls now use `MIN_MOBILE_TOUCH_TARGET`, and static readiness protects routine type chips, owner quick chips, save buttons, delete routine, and add-event save controls from reverting to padding-only sub-48px tap areas before native accessibility traversal is available.
...
- PASS: Avatar Studio owner-input controls now use the shared `MIN_MOBILE_TOUCH_TARGET` floor for scan gallery/camera actions, template tiles, accessory tiles, and mood preview chips, and face-marking options expose explicit screen-reader labels before native accessibility traversal is available.
- LIMIT: full workspace typecheck/build still does not run locally through `pnpm run build:ci` because the root `preinstall` script invokes `sh`, which is not available on PATH in this Windows shell. The focused zero-dependency tests and PixelLab verifier remain the reliable local gates here.
- REMOTE CI: GitHub Actions `WoofWatcher Verify` runs continue to fail before job start or without executing workflow steps with GitHub's account billing/spending-limit blocker; latest checked run `28414398931` for commit `00f8f34` completed in 4 seconds, job `84194078005` had `steps: []`, and the failed-job log was absent (`log not found: 84194078005`). Use the Actions run list as live evidence instead of treating this static doc as current CI state.
...
- Latest remote check, 2026-06-22: Run `27960582957` for commit `448193e` failed in 5 seconds with the same pre-execution shape; job `82740673249` had `steps: []` and `gh run view --log-failed` returned `log not found: 82740673249`.
- Latest remote check, 2026-06-22: Run `27971537534` for commit `f02d52f` failed in 5 seconds with the same pre-execution shape; job `82778705710` had `steps: []` and `gh run view --log-failed` returned `log not found: 82778705710`.
- Latest remote check, 2026-06-22: Run `27991396683` for commit `2df98bf` failed in 5 seconds with the same pre-execution shape; job `82844390957` had `steps: []` and `gh run view --log-failed` returned `log not found: 82844390957`.
...
- Latest remote check, 2026-06-27: Run `28282622455` for commit `e5741cd` failed in 4 seconds with the same pre-execution shape; job `83801094423` had `steps: []` and `gh run view --log-failed` returned `log not found: 83801094423`.
- Latest remote check, 2026-06-29: Run `28397976106` for commit `284dc0b` failed in 3 seconds with the same pre-execution shape; job `84141617033` had `steps: []` and `gh run view --log-failed` returned `log not found: 84141617033`.
...
- Mobile display-name copy for active-household member identity, including row text, edit-modal copy, and save confirmation that name the selected pack instead of implying a global all-households change.
- Household invite joins, active-household persistence, active-household switching, and invite-code visibility across the API and mobile More UI, including direct user provisioning, invited-household care-state readiness, duplicate membership avoidance, default-pack avoidance for first-time invite accepts, joined-pack activation for later care sync routes, successful join confirmation that names the active sync pack, membership-scoped household switching, owner/admin-only invite-code exposure, `/me.households`, and accessible active-pack selection.
- Household audit review across the API, including durable household-scoped audit rows, sensitive household action producers, owner/admin-only list access, safe query normalization, lifecycle/action filtering, newest-first ordering, and OpenAPI/zod/generated React client coverage before final retention policy exists.
- Household member role updates across the API, including owner/admin-only access, active-household member scoping, owner-demotion refusal, durable role-change audit events with target caregiver identity, and OpenAPI/zod/generated React client coverage before final caregiver administration exists.
- Mobile household member role management, including generated role-update hook wiring, existing-member-only role chips, selected/disabled accessibility state, `/me` refresh, Pack Audit refetch, and truthful provider-gated copy for owner transfer, member removal, invite approval, and final permission policy.
- Household Access role permission summaries for owner, admin, sitter, trainer, and vet viewer, plus Mobile More wiring that displays those summaries without leaking internal role ids.
- Pack Audit role-change review details, including target caregiver identity, owner-readable role labels, previous-to-new role transitions, fallback copy for older role-change rows, and screen-reader labels that avoid internal role ids.
- Vet viewer read-only API enforcement for shared care writes, including `PUT /care-state` and `POST/PATCH/DELETE /care-entries` returning 403 while read/review routes remain available.
- Split care-plan versus care-log write enforcement, including `PUT /care-state` using owner/admin/member roles, `POST /care-entries` using owner/admin/member/sitter/trainer roles, and sitter/trainer patch/delete corrections scoped to their own `caregiverUserId`.
...
- Avatar Studio owner-input touch-target and label readiness for scan gallery/camera actions, template tiles, face-marking choices, accessory tiles, and mood preview chips.
...
71. Share a Records Progress Report after adding shared mood check-ins; confirm the report includes the Mood & Energy snapshot, energy counts, latest caregiver/context, and owner-reported/non-diagnostic boundary, while private/stale mood logs stay out.
72. Confirm the shared Progress Report is saved in Report History as a Progress Report artifact with resend and printable-source actions, a stable `.html` filename, escaped care content, and the Mood & Energy boundary preserved.
73. Add receipt and document records with and without local attachments; confirm Records Vault shows per-section attached counts, missing local file titles, and the local-only storage boundary without claiming cloud document storage is ready.

## Missing QA

- Simulator/device runtime smoke. CI web export smoke exists, but it does not replace native runtime rendering.
- Live API integration tests against a test database and provider-auth harness. Focused API contract readiness now covers route scoping/concurrency/query/invite-code/active-household switching/audit-review wiring and sensitive household audit producers without requiring `DATABASE_URL`, Clerk, or installed Express/Drizzle dependencies in this Codex checkout.
