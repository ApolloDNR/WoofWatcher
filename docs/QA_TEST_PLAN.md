...
- Household member role updates across the API, including owner/admin-only access, active-household member scoping, owner-demotion refusal, durable role-change audit events, and OpenAPI/zod/generated React client coverage before final caregiver administration exists.
...
- Pack Audit role-change review details, including owner-readable role labels, previous-to-new role transitions, fallback copy for older role-change rows, and screen-reader labels that avoid internal role ids.
