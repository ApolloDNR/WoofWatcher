...
- PASS: Mobile readiness now protects Setup from regressing to a silent redirect after save by requiring the `buildSetupConfirmation(savedDoc)` and `Alert.alert(...)` path before the owner returns to Today.
