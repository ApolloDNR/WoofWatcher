...
- PASS: Mobile readiness now protects the More Pack Audit surface. The Care Team area uses the generated `useListHouseholdAuditEvents` hook, shows latest owner/admin household trust events, supports event-type and lifecycle filters against the existing audit-events query contract, summarizes stored creation/rename/switch/join details in owner-readable rows and screen-reader labels, and keeps loading, filtered-empty, empty, and offline/error states truthful while lifecycle changes and retention policy remain provider-gated.
