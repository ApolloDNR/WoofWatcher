...
- Live API integration harness for concurrent care-state writes, care-entry delete retention, role-gated household rename, household audit review, sensitive household audit producers, invite-code visibility, invite-join provisioning, active-household persistence after invite accept, `/me.households`, active-household switching, and the mobile switcher path. Focused readiness now protects the static API/mobile contracts and audit producer wiring, but a real test database and provider-auth harness are still required before calling those paths live-verified.
