...
| Caregiver/cloud foundation | Scoped Care Passes, invite drafts, role/scope access model, and provider-neutral cloud sync readiness exist as tested pure functions. | `src/woof-privacy-cloud.js`, `test/woof-privacy-cloud.test.mjs`, `docs/CLOUD_CAREGIVER_FOUNDATION.md`. | Proven locally, not connected |
| Backend operations foundation | Audit events with redaction, talk-to-log drafts, hosted nudge job plans, and report artifact packages exist as tested pure functions and are exposed through the product view model. | `src/woof-operations.js`, `test/woof-operations.test.mjs`, `src/woof-product-view-model.js`, `docs/BACKEND_OPERATIONS_FOUNDATION.md`. | Proven locally, not connected |
| Backend schema foundation | Provider-neutral table, index, access policy, migration, sync policy, and seed draft contracts exist as tested pure functions. | `src/woof-backend-schema.js`, `test/woof-backend-schema.test.mjs`, `src/woof-product-view-model.js`, `docs/BACKEND_SCHEMA_PLAN.md`. | Proven locally, not connected |
...
| Vercel/Replit deployability | `vercel.json` excludes `/api/*` from SPA fallback; `.replit` exists; deployment/handoff notes exist. No WoofWatcher hosted project is live yet. | `vercel.json`, `.replit`, deployment config test, Replit handoff. | Source-ready, deployment gated |
