...
const nextActions = [
  ...
  "Open /care-twin-qa?qaSurface=route-visual-consistency and capture Home, Log, Plans, Health, More, Story & Progress, Records, and Care Team & Supplies on iOS and Android with route-named evidence before claiming route visual proof.",
...
const universalNavigationManifestPath = join(
  mobileRoot,
  "lib",
...
const launchProviderProfilePath = join(
  mobileRoot,
  "lib",
  "launchProviderProfile.ts",
);
...
const recordsFileShareActionsPath = join(
  mobileRoot,
  "lib",
  "recordsFileShareActions.ts",
);
const privacySafetyPath = join(mobileRoot, "lib", "privacySafety.ts");
const careTwinQaRoutePath = join(
  mobileRoot,
  "components",
  "owner",
...
const privacyDataScreenPath = join(
  mobileRoot,
  "components",
  "more",
  "PrivacyDataScreen.tsx",
);
const recordsScreenPath = join(
  mobileRoot,
  "components",
  "health",
  "RecordsScreen.tsx",
);
const recordsOwnerProviderRuntimePath = join(
  mobileRoot,
  "lib",
  "recordsOwnerProviderRuntime.ts",
);
const recordsConsumerProviderRuntimePath = join(
  mobileRoot,
  "lib",
...
const launchProviderSetupSource = existsSync(launchProviderSetupPath)
  ...
  : "";
const launchProviderProfileSource = existsSync(launchProviderProfilePath)
  ? readFileSync(launchProviderProfilePath, "utf8")
  : "";
...
const reportGeneratedBinaryArtifactSource = existsSync(
  ...
  : "";
const recordsFileShareActionsSource = existsSync(recordsFileShareActionsPath)
  ? readFileSync(recordsFileShareActionsPath, "utf8")
  : "";
...
const premiumRouteSource = existsSync(premiumRoutePath)
  ...
  : "";
const privacyDataScreenSource = existsSync(privacyDataScreenPath)
  ? readFileSync(privacyDataScreenPath, "utf8")
  : "";
const recordsScreenSource = existsSync(recordsScreenPath)
  ? readFileSync(recordsScreenPath, "utf8")
  : "";
const recordsOwnerProviderRuntimeSource = existsSync(
  recordsOwnerProviderRuntimePath,
)
  ? readFileSync(recordsOwnerProviderRuntimePath, "utf8")
  : "";
const recordsConsumerProviderRuntimeSource = existsSync(
  recordsConsumerProviderRuntimePath,
)
  ? readFileSync(recordsConsumerProviderRuntimePath, "utf8")
  : "";
...
const authSetupProofManifestIsSourceBacked =
  ...
  /const authSetupProofManifest = ownerOps\s*\?\s*buildAuthSetupProofManifest\(\)\s*:\s*null/.test(
    authUiSource,
  ) &&
  /\{ownerOps && authSetupProofManifest \? \([\s\S]{0,1400}Auth\/Setup proof manifest/.test(
    authUiSource,
  ) &&
  ...
  /\{ownerOps \? \(\s*<BoardCard style=\{s\.authSetupProofCard\}>[\s\S]{0,300}title="Auth\/Setup proof manifest"/.test(
    setupRouteSource,
  ) &&
...
const carePassStorageProofGuardIsSourceBacked =
  ...
  includesAll(recordsOwnerProviderRuntimeSource, [
    "deriveLaunchProviderSetup",
    "storageProviderConfigured",
    "storageProviderEvidence",
  ]) &&
  includesAll(recordsConsumerProviderRuntimeSource, [
    "storageProviderConfigured: false",
    "storageProviderEvidence: null",
  ]) &&
  /storageProviderConfigured:\s*ownerOps\s*\?\s*recordsOwnerProviderRuntime\.storageProviderConfigured\s*:\s*false/.test(
    recordsScreenSource,
  ) &&
  /storageProviderEvidence:\s*ownerOps\s*\?\s*recordsOwnerProviderRuntime\.storageProviderEvidence\s*:\s*null/.test(
    recordsScreenSource,
  ) &&
  /const storageProviderBacked =\s*ownerOps\s*&&\s*storage\.providerBacked/.test(
    recordsScreenSource,
  );
check(
...
const attachmentStorageProofGuardIsSourceBacked =
  ...
  /storageProviderConfigured:\s*Boolean\(\s*launchProviderSetupPlan\.providerInput\.storageProviderConfigured,?\s*\)/.test(
    moreRouteSource,
  ) &&
  /storageProviderProofReady:\s*Boolean\(\s*launchProviderSetupPlan\.providerInput\.storageProviderProofReady,?\s*\)/.test(
    moreRouteSource,
  ) &&
...
const ownerLaunchProviderProfileGuardIsSourceBacked =
  /const ownerLaunchProviderProfile =\s*ownerOps\s*\?\s*state\.launchProviderProfile\s*:\s*null/.test(
    moreRouteSource,
  ) &&
  /deriveLaunchProviderSetup\(ownerLaunchProviderProfile\)/.test(
    moreRouteSource,
  );
...
const aggregateLaunchReadinessProofGuardIsSourceBacked =
  ...
  ownerLaunchProviderProfileGuardIsSourceBacked &&
  includesAll(moreRouteSource, [
    "privacyLegalProofReady",
    "supportRunbookProofReady",
  ]) &&
  [
    ...
    new RegExp(
      `${field}:\\s*Boolean\\(\\s*launchProviderSetupPlan\\.providerInput\\.${field},?\\s*\\)`,
    ).test(moreRouteSource),
  ) &&
...
const careDocLaunchProofPersistenceGuardIsSourceBacked =
  ...
  includesAll(launchProviderProfileSource, [
...
const recordsBinaryExportProofManifestIsSourceBacked =
  ...
  /storageProviderConfigured:\s*input\.providerRuntime\.storageProviderConfigured/.test(
    recordsOwnerProviderRuntimeSource,
  ) &&
  /const binaryProofManifest = ownerOps\s*\?\s*buildRecordsOwnerBinaryProofManifest\(\{[\s\S]{0,900}providerRuntime: recordsOwnerProviderRuntime,[\s\S]{0,120}\}\)\s*:\s*null/.test(
    recordsScreenSource,
  ) &&
  /\{ownerOps\s*&&\s*binaryProofManifest\s*\?\s*\([\s\S]{0,700}Export manifest/.test(
    recordsScreenSource,
  );
check(
...
const generatedBinaryArtifactExportsAreSourceBacked =
  ...
  /const shareGeneratedCarePassPdfArtifact[\s\S]{0,700}const source = buildCarePassPdfArtifactSource\([\s\S]{0,400}await shareGeneratedBinaryArtifactFile\(source/.test(
    recordsScreenSource,
  ) &&
  /const shareCredentialPngArtifact[\s\S]{0,300}shareGeneratedBinaryArtifactFile\(buildCredentialPngSource\(\)/.test(
    recordsScreenSource,
  ) &&
...
const woofGuideAiProviderProofManifestIsSourceBacked =
  ...
  /structured proof\s*files/.test(careTwinQaRouteSource) &&
  includesAll(aiProviderProofSource, [
    "buildAiProviderProofManifest",
    "aiProviderEvidence",
...
const accountDeletionProofManifestIsSourceBacked =
  ...
  /structured route, export, receipt, audit, recovery,\s*and legal\/store proof files/.test(
    careTwinQaRouteSource,
  ) &&
  includesAll(accountDeletionProofSource, [
    "buildAccountDeletionProofManifest",
    "accountDeletionEvidence",
...
const routeVisualProofTargetIsSourceBacked =
  ...
  includesAll(betaHandoffPacketSource, [
    ...
    "Capture Home, Log, Plans, Health, More, Story & Progress, Records, and Care Team & Supplies on iOS and Android before claiming route visual proof.",
    "Name or save each Route Visual screenshot with the canonical route label and platform before attaching it",
    "Home-iOS",
    "Care-Team-Supplies-Android",
  ]) &&
  includesAll(mobileReleaseSmokeChecklistSource, [
...
const routeVisualProofManifestIsSourceBacked =
  ...
  /Native route screenshots stay required before visual\s*sign-off\./.test(
    careTwinQaRouteSource,
  );
check(
