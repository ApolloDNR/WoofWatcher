...
const payload = {
  ...
  nextActions: [
    ...
    "Use macOS, TestFlight, Expo dev client, or a physical iOS device for iOS screenshots; this environment cannot produce iOS simulator proof.",
    ...
    "Open /care-twin-qa?qaSurface=care-entry-provider-sync-proof and use the Care-entry provider sync proof manifest for structured Supabase project, migration/backfill, active-household RLS, retention/export/deletion, dependency-build, and mobile full-refresh sign-off files with MIME, byte size, route-specific denied reads, row count, CI run id, native QA reference, rollback plan, and row-specific booleans before enabling incremental care-entry sync.",
    "Open /care-twin-qa?qaSurface=woofguide-ai-provider-proof for OpenAI key location, structured OpenAI key storage, model-policy, source/citation rules, owner-review write gate, veterinary-safety, and fallback/incident handling proof files with MIME, byte size, and row-specific approvals before enabling live AI.",
