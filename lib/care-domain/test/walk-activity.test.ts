...
test("prompts a walk check when no visible walk is logged", () => {
  ...
  // Without a petName the copy uses the app-wide neutral fallback.
...
test("walk nextStep uses the renamed dog's name, never Phoenix", () => {
  ...
  // The stored "My Dog" placeholder resolves to the same neutral fallback
  // used by resolvePetName everywhere else in the app.
