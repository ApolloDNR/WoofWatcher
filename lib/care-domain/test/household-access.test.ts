...
test("labels launch caregiver roles with truthful permission boundaries", () => {
  const plan = deriveHouseholdAccessPlan({
    household: { name: "Phoenix House", inviteCode: "PHX123" },
    members: [
      { displayName: "Apollo", email: "apollo@example.com", role: "owner" },
      { displayName: "Emma", email: "emma@example.com", role: "admin" },
      { displayName: "Jordan", email: "jordan@example.com", role: "sitter" },
      { displayName: "Maya", email: "maya@example.com", role: "trainer" },
      { displayName: "Dr. Lee", email: "vet@example.com", role: "vet_viewer" },
    ],
    caregivers: [],
    routines: [{ label: "Dinner", owner: "Jordan" }],
  });

  assert.deepEqual(
    plan.people.map((person) => ({ name: person.name, role: person.role, permissions: person.permissions })),
    [
      {
        name: "Apollo",
        role: "Owner",
        permissions: ["Manage household", "Edit care plan", "Log care", "View reports"],
      },
      {
        name: "Emma",
        role: "Admin",
        permissions: ["Manage caregiver roles", "Edit care plan", "Log care", "View reports"],
      },
      {
        name: "Jordan",
        role: "Sitter",
        permissions: ["Log care", "View routines", "Preview sitter handoffs"],
      },
      {
        name: "Maya",
        role: "Trainer",
        permissions: ["Log training", "View behavior notes", "Preview trainer reports"],
      },
      {
        name: "Dr. Lee",
        role: "Vet viewer",
        permissions: ["View reports", "Review health context", "No care-log edits"],
      },
    ],
  );
});
