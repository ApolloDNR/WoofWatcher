...
export function createCaregiverInviteDraft(input = {}, now = new Date().toISOString()) {
  ...
  return {
    ...
    scopes,
    privacyNotice: `This invite would grant access to private ${petName} care context. Send only to trusted people after the household privacy decision is made.`,
...
export function buildCaregiverAccessModel(input = {}, now = new Date().toISOString()) {
  ...
  return {
    ...
    inviteDrafts: [
      createCaregiverInviteDraft({ recipientName: "Weekend sitter", role: "sitter", petName }, now),
      createCaregiverInviteDraft({ recipientName: "Veterinarian", role: "vet", petName }, now),
      createCaregiverInviteDraft({ recipientName: "Trainer", role: "trainer", petName }, now)
    ],
    privacyBoundary: `WoofWatcher cloud access should be invite-only. ${petName} care, health, diet, and household notes are private by default.`
  };
}
