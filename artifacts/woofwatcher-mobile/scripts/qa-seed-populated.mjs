 ...
 *
 * Build the consumer candidate profile before capture:
...
for (let d = 13; d >= 0; d--) {
  ...
  if (d % 7 !== 3)
    entries.push({
      ...
      caregiver: cg,
      occurredAt: at(8, 10),
      durationMinutes: 20 + (d % 4) * 8,
      details: { place: "River Loop" },
    });
  if (d % 2 === 0)
    entries.push({
      id: id(),
      ...
      caregiver: cg,
      occurredAt: at(19),
      durationMinutes: 15 + (d % 3) * 6,
      details: { place: "Neighborhood" },
    });
  if (d % 3 === 0)
    entries.push({
      id: id(),
  ...
  entries.push({
    ...
    details: { energyLevel: d % 3 === 0 ? "high" : "steady" },
  });
  if (d % 4 === 0)
    entries.push({
      id: id(),
      ...
      caregiver: cg,
      occurredAt: at(9),
      amount: `${(28.5 - d * 0.05).toFixed(1)} lb`,
      details: { value: 28.5 - d * 0.05, unit: "lb" },
    });
}
...
for (const [name, route, settle] of POPULATED_ROUTES) {
...
console.log(
  `done -> ${OUT} (${POPULATED_ROUTES.length - misses.length}/${POPULATED_ROUTES.length} routes; ${browserErrorCount(diagnostics.errors)} errors)`,
);
