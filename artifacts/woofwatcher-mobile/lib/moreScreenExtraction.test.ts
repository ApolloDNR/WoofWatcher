...
test("extracts Adventure as the tabbed owner without moving its accepted writes", () => {
  ...
  assert.doesNotMatch(
    source,
    /Stack\.Screen|useLocalSearchParams|getStandaloneRouteBottomPadding/,
  );
  assert.deepEqual(
    [
      calls(source, "addEntry"),
      calls(source, "deleteEntry"),
      calls(source, "updateCareDoc"),
    ],
    [2, 1, 1],
  );
});
...
test("extracts WoofGuide as a prompt-injected tabbed owner", () => {
  ...
  assert.match(
    source,
    /export interface WoofGuideScreenProps\s*{[\s\S]*prompt\?:\s*string;[\s\S]*onBack:\s*\(\)\s*=>\s*void/,
  );
  assert.equal(source.match(/<FlatList\b/g)?.length ?? 0, 1);
  assert.match(
    source,
    /<BoardRouteHeader[\s\S]{0,180}back[\s\S]{0,180}onBack=\{onBack\}[\s\S]{0,180}title="WoofGuide"/,
  );
  assert.match(source, /surface:\s*"tabbed"/);
  ...
  assert.doesNotMatch(
    source,
    /useLocalSearchParams|surface:\s*"standalone"|getStandaloneRouteBottomPadding|getDockedComposerBottomPadding/,
  );
  assert.deepEqual(
    [calls(source, "addEntry"), calls(source, "updateCareDoc")],
    [1, 1],
  );
});
...
test("extracts Privacy and Legal as typed tabbed owners", () => {
  ...
  assert.match(
    privacy,
    /onOpenLegal:\s*\(document:\s*"privacy"\s*\|\s*"terms"\)\s*=>\s*void/,
  );
  ...
  assert.doesNotMatch(
    privacy,
    /useLocalSearchParams|getStandaloneRouteBottomPadding|router\.(?:push|replace)\("\/legal/,
  );
  assert.match(
    privacy,
    /router\.push\(\s*"\/care-twin-qa\?qaSurface=support-legal-readiness-proof"\s+as never,?\s*\)/,
  );
...
test("keeps Settings truthful and non-mutating", () => {
  ...
  assert.match(settings, /title="How WoofWatcher Works"/);
  assert.match(
    settings,
    /export interface SettingsScreenProps\s*{\s*onBack:\s*\(\)\s*=>\s*void;\s*onOpenSection:\s*\(section:\s*Exclude<MoreSection,\s*"root">\)\s*=>\s*void;\s*}/,
  );
  for (const copy of [
  ...
  for (const section of ["dog-profile", "avatar-studio", "privacy"] as const)
    assert.ok(settings.includes(`onOpenSection("${section}")`));
  assert.equal(settings.match(/onOpenSection\("/g)?.length ?? 0, 3);
  assert.match(
    settings,
    /<BoardRouteHeader[\s\S]{0,180}back[\s\S]{0,180}onBack=\{onBack\}/,
  );
  assert.doesNotMatch(
    settings,
    /Switch|AsyncStorage|useCare|useMutation|updateCareDoc|router\.|"\/setup"|"\/health"|onboarding/i,
  );
});
...
test("reduces every moved legacy route to a resolver Redirect", () => {
  for (const route of ["adventure", "woofguide", "privacy", "legal"]) {
    ...
    assert.doesNotMatch(
      source,
      /useCare|useAvatar|useState|StyleSheet|ScrollView|updateCareDoc|addEntry|deleteEntry/,
    );
  }
});
