...
function entry(overrides: Partial<PottyLogDetailEntryLike> = {}): PottyLogDetailEntryLike {
  return {
    id: "potty-1",
    type: "potty",
    title: "Potty break",
...
test("exposes the launch potty detail option set for the log detail sheet", () => {
  assert.deepEqual(POTTY_DETAIL_OUTCOMES.map((option) => option.id), ["pee", "poop", "both", "tried-nothing", "accident"]);
  assert.deepEqual(POTTY_LOCATION_OPTIONS.map((option) => option.id), ["outside", "inside"]);
  assert.ok(POTTY_PEE_DETAIL_OPTIONS.some((option) => option.id === "straining"));
  assert.ok(POTTY_STOOL_CONDITION_OPTIONS.some((option) => option.id === "diarrhea"));
});
...
test("updates a parent potty attempt with pee and stool detail without losing household context", () => {
  ...
  assert.match(String(patch.details.auditTrail?.[0]?.summary), /Apollo updated potty detail on "Potty break" to Pee & poop/);
  assert.deepEqual(patch.details.auditTrail?.[0]?.changes, ["pottyOutcome", "pottyWhere", "condition", "peeDetail", "outcomeAt"]);
});
...
test("marks inside accidents as watch items while keeping the record non-diagnostic", () => {
  ...
  assert.match(String(patch.details.auditTrail?.[0]?.summary), /updated potty detail/);
});
