...
export const RECORDED_LIVE_PREVIEW_HANDOFF_PROOF: MobileLivePreviewHandoffProof = {
  title: "WoofWatcher Live Preview Handoff Proof",
  generatedAtIso: "2026-07-03T22:21:21.304Z",
  result: "PASS",
...
export function buildBetaHandoffPacketShareText(
  ...
  return [
    ...
    "- Capture local Care Pass PDF bytes, local Dog ID PNG bytes, native share/reopen proof, iOS/Android artifact proof, and a structured provider storage proof file with locator, MIME, byte size, bucket names, signed upload/download, household scope, retention/export/deletion, QA evidence storage, and approvals before claiming PDF/PNG readiness.",
    "- Open focused care-entry provider sync target: /care-twin-qa?qaSurface=care-entry-provider-sync-proof.",
    "- Attach structured care-entry provider proof files before enabling incremental sync: Supabase project id proof; migration/backfill proof for care_entries.updated_at and care_entry_tombstones with row count and existing-rows-backfilled; active-household RLS cursor/tombstone proof for /care-entries?updatedSince= and /care-entries/tombstones?updatedSince= with denied cross-household reads; retention/export/deletion policy proof; dependency-complete build proof with CI URL and run id; and mobile full-refresh sign-off with native QA reference and rollback plan. Each file must include file name or URI, MIME, byte size, and row-specific booleans or approvals.",
    "- Open focused WoofGuide AI provider target: /care-twin-qa?qaSurface=woofguide-ai-provider-proof.",
    "- Attach OpenAI key location, approved model policy, source/citation rules, and owner-review write gate, plus veterinary safety boundary and fallback/incident handling before enabling live AI. Each item must be backed by structured WoofGuide AI provider proof files: OpenAI secret storage proof with secret storage, environment scope, rotation owner, local-placeholder exclusion, MIME, and byte size; model-policy, source/citation, owner-review write-gate, veterinary-safety, and fallback/incident handling proof files with row-specific approvals, MIME, and byte size.",
    ...
    "- Attach Plus and Family product ids, billing path decision, iOS App Store and Android Google Play sandbox purchase/renewal/cancel/refund/expired receipt proof, JSON receipt files or URIs with product id, transaction id, byte size, and restorePurchaseConfirmed, entitlement mapping, refund/support policy, and checkout-gate proof before enabling paid checkout.",
    "- Open focused store accounts target: /care-twin-qa?qaSurface=store-accounts-proof.",
    "- Attach platform/store-named proof files before claiming store submission. Attach Apple Developer team id, App Store Connect app record, Google Play package record, bundle/signing ownership, reviewer access/test credentials, screenshots/metadata ownership, and release role approval before claiming store submission; each must be backed by file proof. iOS App Store Connect developer account proof must include role, bundle id, paid-program status, MIME, and byte size; Android Google Play package proof must include package name, Play Console app id, admin role, test track, app signing, MIME, and byte size; shared proof must cover metadata/privacy labels, Apollo release approval, and the no-submit boundary.",
    "- Open focused account deletion target: /care-twin-qa?qaSurface=account-deletion-proof.",
    "- Attach structured account deletion proof files before enabling destructive deletion: deletion-route/auth proof with self-serve route, reauthentication, active-household scope, destructive confirmation, local-preview boundary, MIME, and byte size; export-before-delete proof; data/object deletion receipt proof; audit/support receipt proof; recovery/cancellation proof; and legal/store/Apollo approval proof.",
    ...
    "- Name or save each Route Visual screenshot with the canonical route label and platform before attaching it, for example Home-iOS, Home-Android, Log-iOS, Log-Android, Plans-iOS, Plans-Android, Health-iOS, Health-Android, More-iOS, More-Android, Story-Progress-iOS, Story-Progress-Android, Records-iOS, Records-Android, Care-Team-Supplies-iOS, and Care-Team-Supplies-Android.",
