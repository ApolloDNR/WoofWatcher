...
test("surfaces saved report history without claiming native or server-backed export", () => {
  ...
  assert.match(reportAction?.draft?.body ?? "", /Confirm the saved source still has the local evidence this audience needs/);
  assert.match(reportAction?.draft?.body ?? "", /Dog ID fields, receipt\/document attachments, and report sections/);
