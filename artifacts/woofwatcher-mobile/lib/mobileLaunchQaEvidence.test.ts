...
test("promotes the owner preview core loop as the primary mission before isolated captures", () => {
  const surfaces: readonly MobileReleaseQaSurface[] = [
    focusedSurfaces[0],
    {
      ...
      verificationSteps: ["Open Today, Plan, Quick Log, Health, and More in order from the primary shell; Log History, Records, and Privacy remain secondary routes."],
      acceptanceCriteria: ["The primary shell never hides the active action or routes to a blank screen."],
...
test("preserves owner preview route-loop details in the capture plan and share script", () => {
  ...
  assert.match(target?.routeChecklist?.[0]?.expected ?? "", /Today primary tab/);
  assert.match(target?.routeChecklist?.[4]?.proof ?? "", /Android Launch Readiness screenshot/);
  assert.match(target?.routeChecklist?.[10]?.expected ?? "", /private care quests/);
  ...
  assert.match(text, /Quick Log \(\/fastlog\): Use the elevated center action/);
  assert.match(text, /Log History \(\/log\): Confirm the history-first timeline/);
  assert.match(text, /Privacy \(\/privacy\): Confirm Privacy & Safety/);
  assert.match(
    text,
    /Pack \(\/pack\): Confirm pets, people, household access, and the Care Pass hub expose clear next actions as a secondary route/,
  );
  assert.match(text, /Story \(\/story\): Confirm care career, adventure trail, memories, walk story, and badges remain reachable as a secondary route/);
...
test("expands route visual consistency into route-by-route native screenshot proof", () => {
  ...
  assert.match(text, /Today \(\/\): Phoenix Room, Care Status/);
  assert.match(
    text,
    /Quick Log \(\/fastlog\): The elevated center action opens a focused quick-log sheet/,
  );
  assert.match(text, /Log History \(\/log\): History-first search/);
  assert.match(text, /Privacy \(\/privacy\): Privacy & Safety keeps/);
...
test("keeps note-required owner preview evidence open until the QA note is written", () => {
  ...
  const sessionWithNote: MobileQaSessionState = {
    ...
    surfaceNotes: {
      "owner-preview-core-loop":
        "Today, Plan, Quick Log, Health, More, Log History, Records, and Privacy were reachable without dead ends. Care Pass Report History storage status stayed truthful.",
    },
  };
