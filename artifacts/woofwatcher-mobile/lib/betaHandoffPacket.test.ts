...
test("builds a 48-hour beta handoff packet from release truth and native QA proof gaps", () => {
  ...
  assert.match(text, /Capture Home, Log, Plans, Health, More, Pack, Story, and Records on iOS and Android before claiming route visual proof/);
  assert.match(text, /Name or save each Route Visual screenshot with the route label/);
  assert.match(text, /Home-iOS.*Plans-iOS.*More-Android.*Records-Android/);
...
test("includes saved QA proof manifest when More shares the beta handoff", () => {
  ...
  const session: MobileQaSessionState = {
    ...
    surfaceNotes: {
      "owner-preview-core-loop": "Home, Log, Plans, Health, More, Pack, Story, Adventure, Records, Avatar Studio, and Care Pass opened.",
    },
    surfaceEvidenceById: {
      "owner-preview-core-loop": [
        {
