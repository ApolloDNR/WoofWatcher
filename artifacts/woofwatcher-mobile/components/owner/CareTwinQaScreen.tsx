...
function iconForNeed(
  need: CareTwinRuntimeQaResult["actualNeed"],
): PixelIconName {
  switch (need) {
    case "activity":
      return "walk";
...
function QaBadge({ label, tone }: { label: string; tone: string }) {
  return (
    <View
      style={[
        s.badge,
        { backgroundColor: `${tone}18`, borderColor: `${tone}55` },
      ]}
    >
      <Text style={[s.badgeText, { color: tone, fontFamily: "Inter_700Bold" }]}>
        {label}
      </Text>
    </View>
  );
    ...
    >
      ...
      {steps.map((step, index) => (
        <View key={`${index}-${step}`} style={s.stepRow}>
          <View
            style={[s.stepNumber, { backgroundColor: `${colors.brandNavy}12` }]}
          >
            <Text
              style={[
                s.stepNumberText,
...
const QA_SCREENSHOT_PLATFORM_OPTIONS: {
  label: string;
  value: QaScreenshotEvidencePlatform;
}[] = [
  { label: "iOS", value: "ios" },
...
function buildQaReturnRoute(target: {
  route: string;
  id?: string;
  surfaceId?: string;
  title: string;
...
function resolveStateAction<T>(current: T, next: React.SetStateAction<T>): T {
...
function CareTwinQaScreenBody() {
  ...
  const [selectedEvidencePlatform, setSelectedEvidencePlatform] =
    useState<QaScreenshotEvidencePlatform>(() =>
      qaScreenshotPlatformForRuntime(),
    );
  const [qaStatusById, setQaStatusByIdFromHydration] = useState<
    Record<string, CareTwinQaReviewStatus>
  >({});
  const [qaNotes, setQaNotesFromHydration] = useState<Record<string, string>>(
    {},
  );
  const [qaEvidenceById, setQaEvidenceByIdFromHydration] = useState<
    Record<string, QaScreenshotEvidence[]>
  >({});
  const [surfaceStatusById, setSurfaceStatusByIdFromHydration] = useState<
    Record<string, MobileReleaseQaReviewStatus>
  >({});
  const [surfaceNotes, setSurfaceNotesFromHydration] = useState<
    Record<string, string>
  >({});
  const [surfaceEvidenceById, setSurfaceEvidenceByIdFromHydration] = useState<
    Record<string, QaScreenshotEvidence[]>
  >({});
  ...
  const [qaSessionSavedAt, setQaSessionSavedAt] = useState<
    string | undefined
  >();
  ...
  const setSurfaceStatusById: typeof setSurfaceStatusByIdFromHydration = (
    next,
  ) => {
    applyRealQaEdit(() => {
      const value = resolveStateAction(
  ...
  const setSurfaceEvidenceById: typeof setSurfaceEvidenceByIdFromHydration = (
    next,
  ) => {
    applyRealQaEdit(() => {
      const value = resolveStateAction(
  ...
  const readyCount = scenarios.filter(
    (result) => result.readiness.layeredReady,
  ).length;
  ...
  const focusedQaTargetTone = focusedQaTarget
    ? focusedQaTarget.statusLabel === "Pass"
      ? colors.sage
      : focusedQaTarget.statusLabel === "Needs tune" ||
          focusedQaTarget.statusLabel === "Pass pending proof"
        ? colors.amber
        : colors.copper
    : colors.mutedForeground;
  const focusedQaEvidence = focusedQaTarget
    ? (surfaceEvidenceById[focusedQaTarget.target.surfaceId] ?? [])
    : [];
    ...
    () =>
      focusedQaTarget?.surface.id === "report-binary-export-proof"
        ? buildReportBinaryExportProofManifest({
            ...
            storageProviderConfigured:
              launchProviderSetupPlan.providerInput.storageProviderConfigured,
            providerStorageEvidence: launchProviderSetupPlan.providerInput
              .storageProviderEvidence
              ? [launchProviderSetupPlan.providerInput.storageProviderEvidence]
              : [],
  ...
  const storeAccountsProofManifest = useMemo(
    () =>
      focusedQaTarget?.surface.id === "store-accounts-proof"
  ...
  const nextBetaTarget =
    nextBetaMission.target ?? betaCapturePlan.nextTargets[0];
  const nextBetaTargetMissingEvidence = nextBetaTarget?.missingEvidence ?? [];
  const nextBetaTargetHasMissingEvidence =
    nextBetaTargetMissingEvidence.length > 0;
  const nextBetaTargetPassPendingProof =
    nextBetaTarget?.status === "pass" && nextBetaTargetHasMissingEvidence;
  const nextBetaSurface = useMemo(
    () =>
      nextBetaTarget
        ? releaseQaSurfaces.find(
            (surface) => surface.id === nextBetaTarget.surfaceId,
          )
        : undefined,
    [nextBetaTarget, releaseQaSurfaces],
  );
  const releaseScreenshotEvidenceComplete =
    mobileReleaseQaScreenshotEvidenceComplete(releaseSummary);
  const releasePlatformEvidenceLabel =
    formatMobileReleaseQaPlatformEvidence(releaseSummary);
  const releaseMissingEvidenceLabel =
    formatMobileReleaseQaMissingEvidence(releaseSummary);
  const selectedEvidencePlatformLabel = qaScreenshotEvidencePlatformLabel(
    selectedEvidencePlatform,
  );
  const attachedEvidenceFiles =
    releaseSummary.attachedScreenshots + qaSummary.attachedScreenshots;
            ...
            appliedHydration =
              qaSessionPersistenceGate.applyHydrationIfCurrent(
                ...
                (state) => {
                  ...
                  setSurfaceEvidenceByIdFromHydration(
                    state.surfaceEvidenceById,
                  );
  ...
  useEffect(() => {
    ...
    qaEvidenceActionGate,
    qaEvidenceById,
    qaNotes,
    qaSessionLoaded,
    qaSessionPersistenceGate,
    qaSessionSaveQueue,
    qaStatusById,
    store,
    surfaceEvidenceById,
  ...
  const clearScreenshotEvidence = (target: {
    ...
    enqueueQaEvidenceOperation(async () => {
      ...
      if (result.status === "partial-failure" && qaScreenMountedRef.current) {
        notifyDialog(
          "Screenshot cleanup incomplete",
  ...
  const shareFocusedTargetChecklist = async () => {
    await runQaReportShare(async () => {
      ...
      const message = buildMobileLaunchQaFocusedTargetShareText(
        focusedQaTarget,
        generatedAtIso,
      );
      ...
      >
            ...
            <View style={s.betaRunMissionHeader}>
              <View style={s.betaRunMissionTitleWrap}>
                <Ionicons
                  name="locate-outline"
                  size={18}
                  color={focusedQaTargetTone}
                />
                <View style={s.betaRunMissionCopy}>
                  <Text
                    style={[
                      s.betaRunMissionKicker,
                      {
                        color: focusedQaTargetTone,
              ...
              <QaBadge
                label={focusedQaTarget?.statusLabel ?? "Missing"}
                tone={focusedQaTargetTone}
              />
            </View>
            {focusedQaTarget ? (
              <>
                ...
                <View style={s.betaRunMissionGrid}>
                  ...
                  >
                    ...
                    <Text
                      ...
                      numberOfLines={1}
                    >
                      {focusedQaTarget.target.priority === "launch-critical"
                        ? "Critical"
                        : "Store prep"}
                    </Text>
                  </View>
                ...
                <View
                  style={[
                    s.focusedQaProofBox,
                    {
                      backgroundColor: `${focusedQaTargetTone}12`,
                      borderColor: `${focusedQaTargetTone}55`,
                    },
                  ]}
                >
                  <Text
                    style={[
                      s.focusedQaProofLabel,
                      {
                        color: focusedQaTargetTone,
                        fontFamily: "Inter_800ExtraBold",
                      },
                  ...
                  {(focusedQaTarget.target.missingEvidence.length
                    ...
                    : [
                        "No missing proof remains for this focused target. Share the QA summary and keep public store approval separate.",
                      ]
                  ).map((item) => (
                    <View key={`focused-${item}`} style={s.betaRunStep}>
                      <View
                        style={[
                          s.betaRunStepDot,
                          { backgroundColor: focusedQaTargetTone },
                        ]}
                      />
                      <Text
                        style={[
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Auth and Setup native proof must stay blocked until
                          Clerk, redirects, iOS/Android Auth gateway
                          screenshots, Setup local-preview proof, household
                          sync, and Apollo approval are attached.
                        </Text>
                      </View>
                      <QaBadge
                        label={
                          authSetupProofManifest.status === "ready"
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        <Text
                          style={[
                            s.routeVisualManifestStatLabel,
                            {
                              color: colors.mutedForeground,
                              fontFamily: "Inter_700Bold",
                            },
                        ...
                        <Text
                          style={[
                            s.routeVisualManifestStatValue,
                            {
                              color: colors.foreground,
                              fontFamily: "Inter_800ExtraBold",
                            },
                        ...
                        >
                          {
                            authSetupProofManifest.rows.filter(
                              (row) => row.status === "ready",
                            ).length
                          }
                          /{authSetupProofManifest.rows.length}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {authSetupProofManifest.rows.map((row) => (
                      <View
                        key={`auth-setup-manifest-${row.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                            borderColor: colors.border,
                          },
                      ...
                      >
                        <View style={s.routeVisualManifestRouteLine}>
                          <Text
                            style={[
                              s.routeVisualManifestRouteName,
                              {
                                color: colors.foreground,
                                fontFamily: "Inter_800ExtraBold",
                              },
                    ...
                    {authSetupProofManifest.blockers.length ? (
                      ...
                      >
                        {authSetupProofManifest.blockers.map((blocker) => (
                          <View
                            key={`auth-setup-blocker-${blocker}`}
                            style={s.betaRunStep}
                          >
                            <View
                              style={[
                    ...
                    >
                      This manifest does not configure Clerk, approve OAuth
                      redirects, prove iOS or Android Auth/Setup screenshots,
                      enable provider-backed household creation, sync invites,
                      clear store approval, launch publicly, or replace Apollo
                      sign-off.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Records local files must stay device-verified until
                          Care Pass Report History local HTML, Dog ID local
                          HTML/SVG, native share-sheet behavior, Android content
                          URI, fallback copy, and generated PDF/PNG/provider
                      ...
                      <QaBadge
                        ...
                        tone={
                          recordsLocalFileHandoffProofManifest.nativeFileProofAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {recordsLocalFileHandoffProofManifest.readyCount}/
                          {recordsLocalFileHandoffProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {recordsLocalFileHandoffProofManifest.items.map((item) => (
                      <View
                        key={`records-local-file-handoff-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                      ...
                      >
                        ...
                        <Text
                          style={[
                            s.routeVisualManifestStatusLine,
                            {
                              color: colors.mutedForeground,
                              fontFamily: "Inter_700Bold",
                            },
                        ...
                        >
                          Evidence:{" "}
                          {item.evidenceAttached.length
                            ? item.evidenceAttached.join(", ")
                            : "Not attached"}
                        </Text>
                      </View>
                    ))}
                    {recordsLocalFileHandoffProofManifest.blockers.length ? (
                      ...
                      >
                        {recordsLocalFileHandoffProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`records-local-file-handoff-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not prove native iOS or Android share
                      sheets, Android content URI handoff, fallback copy,
                      generated PDF/PNG readiness, provider-backed storage,
                      cloud sync, public launch, or Apollo sign-off.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Generated PDF/PNG readiness must stay blocked until
                          local bytes, file names, MIME/size proof, native
                          share/reopen behavior, provider storage policy,
                          iOS/Android artifact proof, and Apollo approval are
                          attached.
                        </Text>
                      </View>
                      <QaBadge
                        label={
                          reportBinaryExportProofManifest.status === "ready"
                            ? "Binary proof approved"
                            : "Binary proof blocked"
                        }
                        tone={
                          reportBinaryExportProofManifest.status === "ready"
                            ? colors.sage
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {
                            reportBinaryExportProofManifest.rows.filter(
                              (row) => row.status === "ready",
                            ).length
                          }
                          /{reportBinaryExportProofManifest.rows.length}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {reportBinaryExportProofManifest.rows.map((row) => (
                      <View
                        key={`report-binary-export-proof-manifest-${row.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {reportBinaryExportProofManifest.blockers.length ? (
                      ...
                      >
                        {reportBinaryExportProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`report-binary-export-proof-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not prove native iOS or Android
                      share/reopen behavior, approve PDF or PNG generators,
                      configure provider storage, upload binary artifacts,
                      satisfy App Store or Play Store review, launch publicly,
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Native route screenshots stay required before visual
                          sign-off.
                        </Text>
                      </View>
                      <QaBadge
                        ...
                        tone={
                          routeVisualProofManifest.status === "ready"
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {routeVisualProofManifest.attachedIosScreenshots}/
                          {routeVisualProofManifest.requiredIosScreenshots}
                        </Text>
                      </View>
                      <View
                        style={[
                      ...
                      >
                        ...
                        >
                          {routeVisualProofManifest.attachedAndroidScreenshots}/
                          {routeVisualProofManifest.requiredAndroidScreenshots}
                        </Text>
                      </View>
                    </View>
                    {routeVisualProofManifest.rows.map((row) => (
                      <View
                        key={`route-visual-manifest-${row.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                      ...
                      >
                        <View style={s.routeVisualManifestRouteLine}>
                          ...
                          <Text
                            style={[
                              s.routeVisualManifestRoutePath,
                              {
                                color: colors.mutedForeground,
                                fontFamily: "Inter_700Bold",
                              },
                    ...
                    {routeVisualProofManifest.blockers.length ? (
                      ...
                      >
                        {routeVisualProofManifest.blockers.map((blocker) => (
                          <View
                            key={`route-visual-blocker-${blocker}`}
                            style={s.betaRunStep}
                          >
                            <View
                              style={[
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          ...
                          until provider proof and native QA sign-off are
                          attached.
                        </Text>
                      </View>
                      <QaBadge
                        ...
                        tone={
                          careEntryProviderSyncProofManifest.incrementalSyncAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {careEntryProviderSyncProofManifest.readyCount}/
                          {careEntryProviderSyncProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {careEntryProviderSyncProofManifest.items.map((item) => (
                      <View
                        key={`care-entry-sync-manifest-${item.key}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {careEntryProviderSyncProofManifest.blockers.length ? (
                      ...
                      >
                        {careEntryProviderSyncProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`care-entry-sync-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not run Supabase migrations, approve
                      RLS, configure retention/export/deletion policy, replace
                      native iOS/Android proof, or enable incremental sync.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          ...
                          until provider, safety, and source structured proof
                          files are attached.
                        </Text>
                      </View>
                      <QaBadge
                        ...
                        tone={
                          woofGuideAiProviderProofManifest.liveAiAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {woofGuideAiProviderProofManifest.readyCount}/
                          {woofGuideAiProviderProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {woofGuideAiProviderProofManifest.items.map((item) => (
                      <View
                        key={`woofguide-ai-provider-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {woofGuideAiProviderProofManifest.blockers.length ? (
                      ...
                      >
                        {woofGuideAiProviderProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`woofguide-ai-provider-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not configure OpenAI, approve model
                      policy, allow automatic care-log writes, replace
                      veterinary safety review, or enable live AI.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Reminder Center must stay local until provider
                          credentials, consent copy, quiet hours, and delivery
                          QA proof are attached.
                        </Text>
                      </View>
                      <QaBadge
                        label={pushNotificationsProofManifest.statusLabel}
                        tone={
                          pushNotificationsProofManifest.reminderDeliveryAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {pushNotificationsProofManifest.readyCount}/
                          {pushNotificationsProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                      ...
                      >
                        ...
                        >
                          {pushNotificationsProofManifest.openCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {pushNotificationsProofManifest.items.map((item) => (
                      <View
                        key={`push-notifications-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {pushNotificationsProofManifest.blockers.length ? (
                      <View
                        style={[
                          s.routeVisualManifestBlockers,
                          {
                      ...
                      >
                        {pushNotificationsProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`push-notifications-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not configure Expo push, APNs,
                      Firebase/FCM, prove iOS or Android delivery, approve
                      prompt/legal copy, or enable reminder notifications.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Paid checkout must stay blocked until product ids,
                          billing path, sandbox receipts, restore,
                          refund/support, and checkout-gate proof are attached.
                        </Text>
                      </View>
                      <QaBadge
                        label={
                          paymentsProviderProofManifest.status === "ready"
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {
                            paymentsProviderProofManifest.rows.filter(
                              (row) => row.status === "ready",
                            ).length
                          }
                          /{paymentsProviderProofManifest.rows.length}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {paymentsProviderProofManifest.rows.map((row) => (
                      <View
                        key={`payments-provider-manifest-${row.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {paymentsProviderProofManifest.blockers.length ? (
                      ...
                      >
                        {paymentsProviderProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`payments-provider-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not configure App Store, Play Store,
                      Stripe, sandbox receipts, restore purchases, tax terms,
                      refund approval, money movement, or Apollo checkout
                      sign-off.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Store submission must stay blocked until
                          platform/store-named Apple and Google account,
                          reviewer, metadata, and release approval proof files
                          are attached.
                        </Text>
                      </View>
                      <QaBadge
                        ...
                        tone={
                          storeAccountsProofManifest.appSubmissionAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {storeAccountsProofManifest.readyCount}/
                          {storeAccountsProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {storeAccountsProofManifest.items.map((item) => (
                      <View
                        key={`store-accounts-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {storeAccountsProofManifest.blockers.length ? (
                      ...
                      >
                        {storeAccountsProofManifest.blockers.map((blocker) => (
                          <View
                            key={`store-accounts-blocker-${blocker}`}
                            style={s.betaRunStep}
                          >
                            <View
                              style={[
                    ...
                    >
                      This manifest does not create store accounts, approve
                      metadata or screenshots, submit to App Review or Play
                      review, clear legal/privacy approval, or enable public
                      launch; generic approval notes do not count without named
                      proof files.
                    </Text>
                  </View>
                ) : null}
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Destructive account deletion must stay blocked until
                          structured route, export, receipt, audit, recovery,
                          and legal/store proof files are attached.
                        </Text>
                      </View>
                      <QaBadge
                        label={accountDeletionProofManifest.statusLabel}
                        tone={
                          accountDeletionProofManifest.destructiveDeletionAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {accountDeletionProofManifest.readyCount}/
                          {accountDeletionProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {accountDeletionProofManifest.items.map((item) => (
                      <View
                        key={`account-deletion-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {accountDeletionProofManifest.blockers.length ? (
                      ...
                      >
                        {accountDeletionProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`account-deletion-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not enable destructive deletion, delete
                      provider data or storage objects, approve privacy/legal
                      language, satisfy App Store or Play Store review, or
                  ...
                  >
                    <View style={s.routeVisualManifestHeader}>
                      <View style={s.routeVisualManifestCopy}>
                        ...
                        >
                          Public launch must stay blocked until support,
                          privacy, refund, veterinary-boundary, deletion
                          escalation, incident-response, and Apollo approval
                          proof are attached.
                        </Text>
                      </View>
                      <QaBadge
                        ...
                        tone={
                          supportLegalReadinessProofManifest.publicLaunchAllowed
                            ? colors.sage
                            : colors.amber
                        }
                      />
                    ...
                    <View style={s.routeVisualManifestStats}>
                      ...
                      >
                        ...
                        >
                          {supportLegalReadinessProofManifest.readyCount}/
                          {supportLegalReadinessProofManifest.totalCount}
                        </Text>
                      </View>
                      <View
                        style={[
                    ...
                    {supportLegalReadinessProofManifest.items.map((item) => (
                      <View
                        key={`support-legal-readiness-manifest-${item.label}`}
                        style={[
                          s.routeVisualManifestRow,
                          {
                            backgroundColor: colors.card,
                    ...
                    {supportLegalReadinessProofManifest.blockers.length ? (
                      ...
                      >
                        {supportLegalReadinessProofManifest.blockers.map(
                          (blocker) => (
                            <View
                              key={`support-legal-readiness-blocker-${blocker}`}
                              style={s.betaRunStep}
                            >
                              <View
                                style={[
                    ...
                    >
                      This manifest does not approve legal or privacy copy,
                      refund policy, support operations, veterinary-boundary
                      language, public launch, App Store or Play Store review,
                      or Apollo sign-off.
                    </Text>
                  </View>
                ) : null}
                <EvidenceCapture
                  ...
                  onAttach={() =>
                    attachSurfaceScreenshot(focusedQaTarget.surface)
                  }
                  onClear={() =>
                  ...
                  >
                    <View style={s.betaRunRouteLoopHeader}>
                      ...
                      <View style={s.betaRunRouteLoopCopy}>
                        ...
                        >
                          Open each route on the exact candidate. Photos
                          attachments remain manual references; record
                          exact-binary device proof outside this library picker.
                        </Text>
                      </View>
                    </View>
                    {focusedQaTarget.target.routeChecklist.map(
                      (routeCheck, index) => (
                        <Pressable
                        ...
                        >
                          <View
                            style={[
                              s.betaRunRouteLoopIndex,
                              { backgroundColor: `${colors.copper}18` },
                            ]}
                          >
                            <Text
                              style={[
                                s.betaRunRouteLoopIndexText,
                                {
                                  color: colors.copper,
                                  fontFamily: "Inter_800ExtraBold",
                                },
                ...
                <VerificationStepList
                  colors={colors}
                  label="Setup first"
                  steps={focusedQaTarget.target.setupSteps.slice(0, 4)}
                />
                <VerificationStepList
                  colors={colors}
                  label="Game-feel checklist"
                  steps={focusedQaTarget.target.verificationSteps.slice(0, 6)}
                />
                <VerificationStepList
                  colors={colors}
                  label="Pass when"
                  steps={focusedQaTarget.target.acceptanceCriteria.slice(0, 4)}
                />
                <View
                  style={[
                    s.betaRunEscalation,
                ...
                <View style={s.betaRunMissionActions}>
                  ...
                  <Pressable
                    ...
                    onPress={() =>
                      router.push(
                        buildQaReturnRoute(focusedQaTarget.target) as never,
                      )
                    }
                    style={({ pressed }) => [
                ...
                <View style={s.betaRunMissionReviewRow}>
                  <Pressable
                    ...
                    disabled={qaEditControlsDisabled}
                    onPress={() =>
                      markSurface(focusedQaTarget.target.surfaceId, "pass")
                    }
                    style={({ pressed }) => [
                      s.betaRunMissionReviewButton,
                      {
                        backgroundColor:
                          focusedQaTarget.target.status === "pass"
                            ? `${colors.sage}1F`
                            : pressed
                              ? `${colors.sage}14`
                              : colors.background,
              ...
              >
                The requested QA surface is not in the current release plan.
                Open the next device mission below or share the QA summary.
              </Text>
            )}
          </BoardCard>
        ) : null}
        ...
        <BoardCard style={s.betaRunCard}>
          <View style={s.betaRunHeader}>
            <View
              style={[s.betaRunIcon, { backgroundColor: `${colors.copper}18` }]}
            >
            ...
            <QaBadge
              ...
              tone={
                betaCapturePlan.openSurfaces === 0 ? colors.sage : colors.amber
              }
            />
          </View>
          <View
          ...
          >
            {(nextBetaTargetMissingEvidence.length
              ...
              : [
                  "Share the QA summary and keep public store approval separate from local beta proof.",
                ]
            ).map((item, index) => (
              <View key={`${index}-${item}`} style={s.betaRunStep}>
                <View
                  style={[
                    s.betaRunStepDot,
                    {
                      backgroundColor: nextBetaTarget
                        ? colors.amber
                        : colors.sage,
                    },
                  ]}
          ...
          {nextBetaTarget ? (
            ...
            >
              <View style={s.betaRunMissionHeader}>
                ...
                <QaBadge
                  label={
                    nextBetaTarget.priority === "launch-critical"
                      ? "Launch critical"
                      : "Store prep"
                  }
                  tone={
              ...
              {nextBetaTargetPassPendingProof ? (
                ...
                >
                  ...
                  >
                    This mission is marked Pass, but it stays open until the
                    missing proof is attached and the Mission note is saved.
                  </Text>
                  {nextBetaTargetMissingEvidence.slice(0, 2).map((item) => (
                    <View
                      key={`proof-gate-${item}`}
                      style={s.betaRunProofGateRow}
                    >
                      <View
                        style={[
                          s.betaRunProofGateDot,
                          { backgroundColor: colors.amber },
                        ]}
                      />
                      <Text
                        style={[
                          s.betaRunProofGateItem,
                          {
                            color: colors.foreground,
                            fontFamily: "Inter_700Bold",
                          },
                ...
                >
                  ...
                  {nextBetaTarget.routeChecklist.map((routeCheck, index) => (
                    <Pressable
                      ...
                      onPress={() =>
                        openRouteLoopCheck(routeCheck, nextBetaTarget)
                      }
                      style={({ pressed }) => [
                        s.betaRunRouteLoopRow,
                        {
              ...
              >
                <View style={s.betaRunMissionNoteHeader}>
                  ...
                  {nextBetaTargetMissingEvidence.some((item) =>
                    item.includes("QA note"),
                  ) ? (
              ...
              <VerificationStepList
                ...
                steps={nextBetaTarget.setupSteps.slice(0, 2)}
              />
              <VerificationStepList
                colors={colors}
                label="Pass when"
                steps={nextBetaTarget.acceptanceCriteria.slice(0, 2)}
              />
              <View
                style={[
                  s.betaRunEscalation,
              ...
              <View style={s.betaRunMissionActions}>
                <Pressable
                  ...
                  onPress={() => {
                    if (nextBetaSurface)
                      attachSurfaceScreenshot(nextBetaSurface);
                  }}
                  style={({ pressed }) => [
                    s.betaRunMissionAttach,
                    {
                ...
                <View style={s.betaRunMissionReviewRow}>
                  <Pressable
                    ...
                    disabled={qaEditControlsDisabled}
                    onPress={() =>
                      markSurface(nextBetaTarget.surfaceId, "pass")
                    }
                    style={({ pressed }) => [
                      s.betaRunMissionReviewButton,
                      {
                        backgroundColor:
                          nextBetaTarget.status === "pass"
                            ? `${colors.sage}1F`
                            : pressed
                              ? `${colors.sage}14`
                              : colors.background,
                  ...
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      ...
                      {
                        backgroundColor:
                          nextBetaTarget.status === "needs-review"
                            ? `${colors.amber}1F`
                            : pressed
                              ? `${colors.amber}14`
                              : colors.background,
          ...
          >
            <View style={s.betaRunPlatformHeader}>
              <View style={s.betaRunPlatformCopy}>
                ...
                >
                  Photos-library attachments are manual self-attested
                  references. A platform tag helps organize them but never
                  closes an exact-binary iOS or Android release gate.
                </Text>
              </View>
              <QaBadge
                label={selectedEvidencePlatformLabel}
                tone={
                  selectedEvidencePlatform === "web"
                    ? colors.amber
                    : colors.sage
                }
              />
                  ...
                  >
                    <Text
                      style={[
                        ...
                        {
                          color: active ? colors.sage : colors.foreground,
                          fontFamily: "Inter_800ExtraBold",
                        },
                      ]}
                    >
          ...
          <View style={s.betaRunActions}>
            <Pressable
              ...
              accessibilityLabel={
                nextBetaTarget
                  ? `Open next beta QA surface: ${nextBetaTarget.title}`
                  : "Share completed beta QA summary"
              }
              accessibilityState={{ disabled: qaReportShareBusy }}
              disabled={qaReportShareBusy}
              onPress={
                nextBetaTarget
                  ? () =>
                      router.push(buildQaReturnRoute(nextBetaTarget) as never)
                  : shareQaSummary
              }
              style={({ pressed }) => [
                s.betaRunPrimary,
            ...
            >
              <Ionicons
                name={nextBetaTarget ? "navigate-outline" : "share-outline"}
                size={17}
                color="#FFF9EF"
              />
              <Text
                style={[
                  s.betaRunPrimaryText,
                  { fontFamily: "Inter_800ExtraBold" },
                ]}
              >
        ...
        <BoardCard style={s.summaryCard}>
          <View style={s.summaryTop}>
            <View
              style={[s.summaryIcon, { backgroundColor: `${colors.sage}18` }]}
            >
            ...
            <View style={s.summaryCopy}>
              ...
              >
                Check launch workflows, screenshot evidence, sprite states,
                phone-size readability, and non-diagnostic tone.
              </Text>
            </View>
          </View>
          <View style={s.summaryGrid}>
            <QaBadge
              label={`${releaseSummary.passed}/${releaseSummary.total} release`}
              tone={
                releaseSummary.passed === releaseSummary.total
                  ? colors.sage
                  : colors.amber
              }
            />
            <QaBadge
              label={`${releaseSummary.passedWithRequiredProof} exact-device`}
              tone={
                releaseSummary.passPendingProof === 0
                  ? colors.sage
                  : colors.amber
              }
            />
            <QaBadge
              label={`${releaseSummary.passPendingProof} release pending`}
              tone={
                releaseSummary.passPendingProof === 0
                  ? colors.sage
                  : colors.amber
            ...
            <QaBadge
              label={
                qaProofManifest
                  ? `Saved manifest ${qaProofManifest.proofId}`
            ...
            <QaBadge
              label={
                qaProofManifest
                  ? `${qaProofManifest.totalEvidenceFiles} saved manifest files`
                  : "No saved manifest"
              }
              tone={
                qaProofManifest && qaProofManifest.totalEvidenceFiles > 0
                  ? colors.sage
                  : colors.amber
              }
            />
            <QaBadge
              label={`${readyCount}/${scenarios.length} layered`}
              tone={
                readyCount === scenarios.length ? colors.sage : colors.amber
              }
            />
            <QaBadge label={`${qaSummary.passed} pass`} tone={colors.sage} />
            <QaBadge
              label={`${qaSummary.passedWithNativeProof} exact-device`}
              tone={
                qaSummary.passPendingProof === 0 ? colors.sage : colors.amber
              }
            />
            <QaBadge
              label={`${qaSummary.passPendingProof} pending proof`}
              tone={
                qaSummary.passPendingProof === 0 ? colors.sage : colors.amber
              }
            />
            <QaBadge
              label={`${qaSummary.needsReview} needs tune`}
              tone={colors.amber}
            />
            <QaBadge
              label={`${qaSummary.unreviewed} unreviewed`}
              tone={colors.mutedForeground}
            />
            <QaBadge
              label={`${attachedEvidenceFiles} evidence files`}
              tone={
                releaseScreenshotEvidenceComplete ? colors.sage : colors.amber
              }
            />
            <QaBadge
              label={
                releaseScreenshotEvidenceComplete
                  ? "Exact-device gate ready"
                  : "Exact-device gate open"
              }
              tone={
                releaseScreenshotEvidenceComplete ? colors.sage : colors.amber
              }
            />
            <QaBadge
              label={`iOS ${releaseSummary.attachedIosScreenshots}/${releaseSummary.requiredIosScreenshots}`}
              tone={
                releaseSummary.missingIosScreenshots === 0
                  ? colors.sage
                  : colors.amber
              }
            />
            <QaBadge
              label={`Android ${releaseSummary.attachedAndroidScreenshots}/${releaseSummary.requiredAndroidScreenshots}`}
              tone={
                releaseSummary.missingAndroidScreenshots === 0
                  ? colors.sage
                  : colors.amber
              }
            />
          ...
          <Text
            style={[
              s.platformEvidenceText,
              {
                color: releaseScreenshotEvidenceComplete
                  ? colors.sage
                  : colors.amber,
                fontFamily: "Inter_700Bold",
              },
          ...
          >
            Exact-device gate: {releasePlatformEvidenceLabel}.{" "}
            {releaseMissingEvidenceLabel}.
          </Text>
          <Text
            style={[
              s.savedSessionText,
          ...
          >
            ...
            <Text style={[s.shareButtonText, { fontFamily: "Inter_700Bold" }]}>
              Share QA summary
            </Text>
          </Pressable>
        </BoardCard>
        ...
        {releaseSurfaces.map((surface) => {
          ...
          const surfaceMissingEvidence =
            mobileReleaseQaMissingEvidenceForSurface(surface, review);
          const surfacePassPendingProof =
            reviewStatus === "pass" && surfaceMissingEvidence.length > 0;
          ...
          return (
            <BoardCard key={surface.id} style={s.surfaceCard}>
              <View style={s.surfaceHeader}>
                <View style={s.surfaceTitleWrap}>
                  ...
                  <View style={s.surfaceTitleCopy}>
                    ...
                    >
                      {surface.route} -{" "}
                      {surface.priority === "launch-critical"
                        ? "Launch critical"
                        : "Release polish"}
                    </Text>
                  </View>
              ...
              <VerificationStepList
                colors={colors}
                label="Setup first"
                steps={surface.setupSteps}
              />
              <VerificationStepList
                colors={colors}
                steps={surface.verificationSteps}
              />
              <VerificationStepList
                colors={colors}
                label="Pass criteria"
                steps={surface.acceptanceCriteria}
              />
              ...
              {surfacePassPendingProof ? (
                ...
                >
                  ...
                  >
                    This surface is marked Pass, but the required screenshot or
        ...
        <BoardSectionHeader
          ...
          accessory={
            <BoardPill
              label={storeSubmissionPacket.verdictLabel}
              tone={colors.copper}
            />
          }
        />
        ...
        <BoardCard style={s.surfaceCard}>
          <View style={s.surfaceHeader}>
            <View style={s.surfaceTitleWrap}>
              ...
              <View style={s.surfaceTitleCopy}>
                ...
                >
                  {storeSubmissionPacket.metadata.subtitle} -{" "}
                  {storeSubmissionPacket.metadata.category}
                </Text>
              </View>
            </View>
            <QaBadge
              label={`${storeSubmissionPacket.screenshotChecklist.length} store screens`}
              tone={colors.copper}
            />
          </View>
          <Text
          ...
          >
            ...
            >
              Capture truthful App Store and Play Store screenshots only. Do not
              show private household data or claim live AI, cloud storage,
              payments, or submission approval until those gates are actually
              closed.
            </Text>
          </View>
                ...
                >
                  <View style={s.betaRunRouteLoopHeader}>
                    ...
                    <View style={s.betaRunRouteLoopCopy}>
                      ...
                      >
                        Open the exact surface, frame the store-safe screenshot,
                        then return here to attach proof.
                      </Text>
                    </View>
                  </View>
        ...
        {scenarios.map((result, index) => {
          ...
          const motionRecipeSummary = describeMotionRecipeForSpriteAction(
            result.actualAction,
          );
          ...
          const scenarioMissingEvidence =
            careTwinQaMissingNativeProof(scenarioReview);
            ...
            <BoardCard key={result.scenario.id} style={s.scenarioCard}>
              <View style={s.scenarioHeader}>
                <View style={s.scenarioTitleRow}>
                  ...
                  >
                    <Text style={[s.indexText, { fontFamily: DISPLAY_SEMI }]}>
                      {index + 1}
                    </Text>
                  </View>
                  <View style={s.scenarioTitleCopy}>
                    ...
                    >
                      {formatSlug(result.actualAction)} in{" "}
                      {formatSlug(result.actualRoomVariant)}
                    </Text>
                  </View>
                </View>
                <QaBadge
              ...
              <View style={s.metaGrid}>
                ...
                <MetaItem
                  icon="locate-outline"
                  label="Zone"
                  value={formatSlug(result.actualZone)}
                />
                <MetaItem
                  icon="pulse-outline"
                  label="Need"
                ...
                <MetaItem
                  ...
                  label="Tap"
                  value={formatSlug(choreography.tapReaction.action)}
                />
              </View>
              ...
              >
                <View style={s.motionProofHeader}>
                  ...
                  <QaBadge
                    label={`${motionRecipe.bodyBobPx}px bob`}
                    tone={colors.sage}
                  />
                </View>
                <View style={s.motionMetricGrid}>
                  <MetaItem
                    ...
                    label="Sway"
                    value={`${motionRecipe.bodySwayPx}px`}
                  />
                  <MetaItem
                    icon="sync-outline"
                    label="Tilt"
                    value={`${motionRecipe.tiltDeg}deg`}
                  />
                  <MetaItem
                    icon="resize-outline"
                    label="Pulse"
                    value={`${motionRecipe.scalePulse}`}
                  />
                  <MetaItem
                    icon="ellipse-outline"
                    label="Shadow"
                    value={`${motionRecipe.shadowScalePulse}/${motionRecipe.shadowOpacityPulse}`}
                  />
                </View>
                <Text
                  style={[
              ...
              >
                ...
                <View style={s.motionMetricGrid}>
                  <MetaItem
                    icon="crop-outline"
                    label="Zone"
                    value={formatSlug(stageFraming.zone)}
                  />
                  <MetaItem
                    icon="layers-outline"
                    label="Avatar"
              ...
              {scenarioPassPendingProof ? (
                ...
                >
                  ...
                  >
                    This care-twin state is marked Pass, but it still needs
                    screenshot evidence bound to an exact iOS or Android binary
                    and device. A Photos attachment alone does not count.
                  </Text>
                  <View style={s.betaRunProofGateRow}>
                    <View
                      style={[
              ...
              <View style={s.reviewGrid}>
                ...
                <ReviewButton
                  ...
                  label="Needs tune"
                  onPress={() =>
                    markScenario(result.scenario.id, "needs-review")
                  }
                  tone={colors.amber}
                />
              </View>
    ...
    >
      ...
      <Text
        style={[
          s.reviewButtonText,
          {
            color: active ? tone : colors.foreground,
            fontFamily: "Inter_700Bold",
          },
        ]}
      >
  ...
  return (
    ...
    >
      <View style={s.evidenceCaptureHeader}>
        ...
        <QaBadge
          label={label}
          tone={evidence.length ? colors.sage : colors.amber}
        />
      </View>
      <Text
        style={[
          s.evidenceCaptureHelp,
          { color: colors.mutedForeground, fontFamily: "Inter_500Medium" },
        ]}
      >
        Attach from Photos as a manual self-attested reference for the route
        notes. New attachments are tagged as {targetPlatformLabel}, but the tag
        does not prove the exact binary, device, or native run.
      </Text>
      {evidence.length ? (
        <View style={s.attachedList}>
          {evidence.map((item) => (
            <View
              key={`${item.uri}-${item.capturedAtIso}`}
              style={[s.attachedItem, { borderColor: colors.border }]}
            >
