...
export default function VariantA() {
  ...
  const pulse = [
    ...
    { icon: "paw" as PulseIconName, label: "Walks", value: `${c.walks.done}/${c.walks.target}`, sub: c.walkMinutes > 0 ? `${c.walkMinutes} min today` : "None yet" },
    { icon: "drop" as PulseIconName, label: "Potty", value: `${c.potty.done}/${c.potty.target}`, sub: c.potty.done > 0 ? "Looking good" : "—" },
    { icon: "star" as PulseIconName, label: "Training", value: `${c.training}`, sub: c.training > 0 ? "Nice work" : "—" },
    { icon: "heart" as PulseIconName, label: "Health", value: c.healthAlert ? "Watch" : "Good", sub: c.healthAlert ? "1 alert to review" : "No alerts" },
  ];
      ...
      >
        <Animated.View style={{ opacity: fade }}>
          ...
          <View style={s.greeting}>
            <Text style={[s.eyebrow, { color: colors.copper, fontFamily: "Inter_600SemiBold" }]}>
          ...
          {/* Painted hero card */}
          <View style={[s.heroCard, { backgroundColor: colors.card, shadowColor: colors.primary, borderColor: colors.border }]}>
            <View style={s.heroImageWrap}>
              <Image source={HERO_IMAGES[status.mood]} style={s.heroImage} resizeMode="cover" />
              <LinearGradient
                colors={["transparent", "rgba(20,30,24,0.04)", "rgba(20,30,24,0.5)"]}
                locations={[0, 0.5, 1]}
                style={s.heroScrim}
              />

              <Pressable onPress={() => router.push("/plans")} style={[s.floatChip, s.nameChip]}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [s.nextUpRow, { opacity: pressed ? 0.6 : 1, borderTopColor: colors.border }]}
            >
          ...
          {/* Today's Pulse — cohesive single panel */}
          ...
          <View style={[s.pulsePanel, { backgroundColor: colors.card, shadowColor: colors.primary, borderColor: colors.border }]}>
            {pulse.map((p, i) => (
              <View
                key={p.label}
                style={[s.pulseRow, i < pulse.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}
              >
                <View style={[s.pulseIconWrap, { backgroundColor: PULSE_COLORS[p.icon] + "16" }]}>
                  <PulseIcon name={p.icon} size={24} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.pulseLabel, { color: colors.foreground, fontFamily: "Inter_600SemiBold" }]}>{p.label}</Text>
                  <Text style={[s.pulseSub, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>{p.sub}</Text>
                </View>
                <Text style={[s.pulseValue, { color: colors.foreground, fontFamily: "Inter_700Bold" }]}>{p.value}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={[s.quickGrid, { gap: GAP }]}>
            {QUICK_LOG.map((item) => (
              ...
              >
                <View style={[s.quickIconWrap, { backgroundColor: PULSE_COLORS[item.icon] + "16" }]}>
                  <PulseIcon name={item.icon} size={24} />
                </View>
...
const s = StyleSheet.create({
  ...
  eyebrow: { fontSize: 12, letterSpacing: 1.2, marginBottom: 6 },
  greetingTitle: { fontSize: 28, letterSpacing: -0.6, lineHeight: 32 },
  ...
  heroImageWrap: { height: 360, position: "relative" },
  ...
  moodValue: { fontSize: 16, marginTop: 2 },
  energyBlock: { flex: 1 },
  ...
  pulseRow: { flexDirection: "row", alignItems: "center", gap: 14, paddingVertical: 14 },
  pulseIconWrap: { width: 44, height: 44, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  pulseLabel: { fontSize: 15 },
  pulseSub: { fontSize: 12.5, marginTop: 2 },
  pulseValue: { fontSize: 18, letterSpacing: -0.3 },
