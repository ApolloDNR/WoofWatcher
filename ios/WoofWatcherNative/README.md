...
- `CareStore.swift`: root-owned observable store with local JSON save/load, editable routines, editable goals, health watch, monthly summary, goal review, and caregiver handoff digest.
- `ContentView.swift`: TabView shell for Today, Schedule, Goals, Log, Health, Records, Report, and Helper, including a shareable Today handoff section.
...
8. Confirm the report and helper boundary match the PWA behavior.
9. Confirm the Today handoff shows next routine, last meal, last walk, follow-ups, and caregiver load.
